...
class AccountKardexReport(models.AbstractModel):
	...
	def _report_expand_unfoldable_line_svl(self, line_dict_id, groupby, options, progress, offset, unfold_all_batch_data=None):
		...
		for line in self._get_svl_lines(options, product_id, report=report, offset=offset):
			if not unfold_all and load_more_counter == report.load_more_limit:
				has_more = True
				break

			lines.append({
				'id': report._get_generic_line_id('stock.valuation.layer', line['id'], parent_line_id=line_dict_id),
				...
				'columns': [
					...
					{'name': report.format_value(line['init_unit_cost'], blank_if_zero=False, figure_type='monetary')},
					{'name': report.format_value(line['init_value'], blank_if_zero=False, figure_type='monetary')},
					# MOVIMIENTO
					{'name': line['quantity']},
					{'name': report.format_value(line['unit_cost'], blank_if_zero=False, figure_type='monetary')},
					{'name': report.format_value(line['value'], blank_if_zero=False, figure_type='monetary')},
					# SALDO FINAL
					{'name': line['end_quantity']},
					{'name': report.format_value(line['end_unit_cost'], blank_if_zero=False, figure_type='monetary')},
					{'name': report.format_value(line['end_value'], blank_if_zero=False, figure_type='monetary')},
				],
			})
			load_more_counter += 1
	...
	def _get_product_ids(self, options):
		return self.env['product.product'].browse([x['product_id'][0] for x in self.env['stock.valuation.layer'].read_group([
	...
	def _caret_options_initializer(self):
		return {'stock.valuation.layer': [{'name': 'Abrir valoración', 'action': 'caret_option_open_record_form'}]}