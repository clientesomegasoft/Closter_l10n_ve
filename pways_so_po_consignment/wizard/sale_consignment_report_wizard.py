...
class SaleConsignmentReportWizard(models.TransientModel):
    ...
    report_type = fields.Selection([('consignment_details_report', 'Consignment Detailes Report'), ('sale_details_report', 'Sale Detailes Report')], default="consignment_details_report", string="Report")
    ...
    consignment_ids = fields.Many2many('sale.consignment.order', string="Consignment Order")
    ...
    def action_print_report(self):
        ...
        return self.env.ref('pways_so_po_consignment.sale_consignment_report').report_action(self, data=data)
...
class SaleConsignmentTemplate(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if report_type == "sale_details_report":
            consignment_domain = [('date_order', '>=', date_from),('date_order', '<=', date_to)]
            if customer_ids:
                consignment_domain.append(('partner_id', 'in', customer_ids))
            consignment_ids = self.env['sale.consignment.order'].search(consignment_domain)
            for consignment in consignment_ids:
                order_line = self.env['sale.consignment.line']
                sale_order_lines = self.env['sale.order.line']
                if product_ids:
                    sale_order_lines = consignment.sale_ids.mapped('order_line').filtered(lambda x: x.product_id.id in product_ids)
                    order_line = consignment.order_line.filtered(lambda x: x.product_id.id in product_ids)
                else:
