...
class SaleOrderCreateWizards(models.TransientModel):
    ...
    create_type = fields.Selection([('manually', 'Manually'),('import_xls', 'Import Xls')], string="Selection", default="manually")
    consignment_line_ids = fields.One2many('order.line.wizard', 'order_id', string="Line Ids")
    ...
    def default_get(self, vals):
        ...
        if consignment_id:
            ...
            if all([line.available_qty == 0 for line in consignment_line_ids]):
                raise ValidationError(_("There are nothing to sale from this consignment order"))
            
            fil_consignment_line_ids = consignment_line_ids.filtered(lambda l: l.available_qty > 0 and l.move_id.state in ['assigned','done'])
    ...
    def action_create_sale_order(self):
        ...
        for line in self.consignment_line_ids.filtered(lambda x: x.product_id.tracking != "none"):
            if not line.order_line_ids:
                raise ValidationError(_("Please select proper lot/serial for product %s." %(line.product_id.name)))
        ...
        vals = {
            ...
            'warehouse_id': consignment_id.warehouse_id and consignment_id.warehouse_id.id,
...
class SaleOrderCreateWizard(models.TransientModel):
    ...
    order_line_ids = fields.One2many('order.line.wizard.lot.serial', 'line_id', string="Order")
    ...
    def sale_line_lot_serial(self):
        ...
        if not self.order_line_ids:
            for line in consignment_line_ids.filtered(lambda x: x.product_id == self.product_id and x == self.consignment_line_id):
                for lot in line.line_lot_serial_ids.filtered(lambda x: x.unreserve_qty > 0):
    ...
    def confirm_lot_serial(self):
        ...
        view_id = self.env.ref('pways_so_po_consignment.sale_order_create_wizard_form_view').id
        return{
            'type': 'ir.actions.act_window',
