...
class ConsignmentReportWizard(models.TransientModel):
    ...
    report_type = fields.Selection(
        [
            ("consignment_details_report", "Consignment Sale Report"),
            ("purchase_sale_report", "Consignment Detail Report"),
        ],
        string="Report",
        default="consignment_details_report",
    )
    consignment_account_id = fields.Many2one(
        "account.analytic.account", string="Consignment Account"
    )
    ...
    def action_print_report(self):
        ...
        return self.env.ref("pways_so_po_consignment.consignment_report").report_action(
            self, data=data
        )
...
class ConsignmentTemplateReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if report_type == "consignment_details_report":
            sale_domain_consignment = [
                ("is_consignments", "=", True),
                ("date_order", ">=", date_from),
                ("date_order", "<=", date_to),
                ("state", "=", "sale"),
                ("analytic_id", "=", consignment_account_id.id),
            ]
            ...
            commission_amount = (
                total_sale_amount * consignment_account_id.commission
            ) / 100
            consignment_lines = consignment_account_id.mapped(
                "consignment_ids"
            ).filtered(lambda x: x.consignment_type == "expense")
