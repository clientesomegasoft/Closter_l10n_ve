...
class StockWarehouse(models.Model):
    ...
    consignment_location_id = fields.Many2one('stock.location', domain="[('usage', '=', 'internal')]", string="Consignment Location")
...
class StockMove(models.Model):
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        stock_move = super(StockMove, self)._action_confirm(merge=True, merge_into=False)
        for move in self.filtered(lambda x: x.sale_line_id.consignment_move_id):
            move_line_ids = []
            move.write({'move_orig_ids': [(4, move.sale_line_id.consignment_move_id.id)]})
