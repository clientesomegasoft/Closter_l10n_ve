...
class SaleOrder(models.Model):
    ...
    consignment_rount_id = fields.Many2one(related='warehouse_id.consignment_rount_id', string="Consignment Route", store=True)

class SaleOrderLine(models.Model):
    _inherit = "sale.order.line"

    product_tracking = fields.Selection(related='product_id.tracking')
    line_lot_serial_ids = fields.One2many('sale.order.line.lot.serial', 'line_id', string="lines")
    consignment_move_id = fields.Many2one('stock.move', string="Consignment Move")
    sale_consignment_line_id = fields.Many2one('sale.consignment.line', string="Consignment Line")
