...
class PurchaseOrder(models.Model):
    ...
    expenses_ids = fields.One2many('expenses.details', 'purchase_expense_id', string="Expenses Details")
    ...
    def create(self,vals):
        ...
        if res['is_consignments']:
            res['name'] = self.env['ir.sequence'].next_by_code('purchase.consignments') or '/'
        return res
    ...
    def button_confirm(self):
        ...
        if self.is_consignments:
            plan_id = self.env['account.analytic.plan'].search([('is_consignment','=','True')], limit=1)
            if not plan_id:
                raise ValidationError('Please set analytic plan for consignment')
            analytic_id = self.env['account.analytic.account'].search([('purchase_order_id', '=', self.id)], limit=1)
            if not analytic_id:
...
class ExpensesDetails(models.Model):
    ...
    def unlink(self):
        for order in self:
            ...
            if analytic_id:
                consignment_id = analytic_id.consignment_ids.filtered(lambda x: x.expenses_id.id == order.id)
