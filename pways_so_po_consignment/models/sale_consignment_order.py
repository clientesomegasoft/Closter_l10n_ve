...
class SaleConsignmentOrder(models.Model):
    ...
    order_line = fields.One2many('sale.consignment.line', 'order_id', string='Order Lines')
    ...
    move_line_count = fields.Integer(string="Moves Lines", compute="_compute_move_line_count")
    ...
    def create(self,vals):
        ...
        res['name'] = self.env['ir.sequence'].next_by_code('sale.consignment.order') or '/'
        return res
    ...
    def action_request_approval(self):
        for line in self.order_line:
            if line.product_id.tracking != "none" and line.product_uom_qty != sum(line.line_lot_serial_ids.mapped('qty')):
                raise ValidationError(_("Please select lot/serial for product %s " %(line.product_id.name)))
    ...
    def action_confirm(self):
        ...
        if not self.warehouse_id.consignment_location_id or not self.warehouse_id.consignment_rount_id:
            raise ValidationError(_("Please assigned consignment location or consignment route in seleted warehouse."))
        
        for line in self.order_line:
            if line.product_id.tracking != "none" and line.product_uom_qty != sum(line.line_lot_serial_ids.mapped('qty')):
                raise ValidationError(_("Please select lot/serail for product %s " %(line.product_id.name)))
        ...
        for consignment in self:
            for line in consignment.order_line:
                ...
                quantity = sum(line.line_lot_serial_ids.mapped('qty')) if line.product_id.tracking != 'none' else line.product_uom_qty
                ...
                # create move
                stock_to_transit = self.env['stock.move'].sudo().create({
            ...
            consignment.write({'state': 'consignment', 'moves_ids' : [(4, x) for x in stock_to_transit_ids.ids]})
    ...
    def action_return_qty(self):
        ...
        for line in order_line_ids:
            ...
            return_move_ids |= self.env['stock.move'].sudo().create({
                    'name': self.name,
                    'origin': self.name,
                    'product_id': line.product_id.id,
                    'location_id': self.warehouse_id.consignment_location_id.id,
            ...
            self.write({'state': 'done', 'moves_ids': [(4, x) for x in return_move_ids.ids]})

    def action_done(self):
        if all([line.available_qty == 0 for line in self.order_line]):
            self.write({'state': 'done'})
        else:
            raise ValidationError(_("Before making done. Please either sale or return products"))

    def action_assign(self):
        for move in self.moves_ids.filtered(lambda x: x.state in ['waiting', 'confirmed', 'partially_available']):
            move._action_assign()

    def _compute_bill_count(self):
        for analytic in self:
            analytic.bill_count = len(self.env['account.move'].search([('consignment_id', '=', self.id)]))
    ...
    def action_generate_bill(self):
        ...
        if income_ids and commission:
            ...
            name = '%s-%s-%s-%s-%s' %(self.partner_id.name, product_commission.name, commission, self.name, sale_name)
            ...
            if consignments_bill:
                consignments_bill.write({'invoice_line_ids': invoice_line_list or False})
...
class SaleConsignmentLine(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company',  default=lambda self: self.env.user.company_id)
    order_id = fields.Many2one('sale.consignment.order', string="sale Consignment Order")
    product_id = fields.Many2one('product.product', string='Product', change_default=True, ondelete='restrict', check_company=True)
    ...
    line_lot_serial_ids = fields.One2many('sale.consignment.line.lot.serial', 'line_id', string="Sale Consignment Line Lot Serial")
    warehouse_id = fields.Many2one('stock.warehouse', related='order_id.warehouse_id', store=True)
    ...
    def _compute_free_qty(self):
        for line in self:
            ...
            available_quantity = line.product_id.with_context({}, warehouse=warehouse_id).free_qty
            line.free_qty = available_quantity

    def _compute_available_qty(self):
        for line in self:
            order_lines = line.order_id.sale_ids.mapped('order_line')
            order_lines_product = order_lines.filtered(lambda x: x.product_id.id == line.product_id.id and x.sale_consignment_line_id.id == line.id)
            available_qty = line.product_uom_qty - sum(order_lines_product.mapped('product_uom_qty'))
    ...
    def sale_line_lot_serial(self):
        view_id = self.env.ref('pways_so_po_consignment.sale_consignment_line_form_view').id
        return {
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_id': view_id,
