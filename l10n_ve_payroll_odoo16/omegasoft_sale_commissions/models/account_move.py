...
class AccountMove(models.Model):
	...
	def action_post(self):
		...
		for rec in self:
			...
			total_invoice = rec.currency_id._convert(rec.amount_untaxed, rec.company_currency_id, rec.company_id, rec.currency_rate_ref)
			#Se obtienen las líneas de configuración que cumplan con las condiciones establecidas
			query_resul = rec._query_commission_conf_line(rec.department_id.id, rec.seller_employee_id.id, rec.assigned_employee_id.id, "'posted_bill'", total_invoice)
			...
			if (query_resul and commission_products):
				rec._create_paid_commission_line(query_resul, commission_products, total_invoice)
		return res

	@api.constrains('payment_state')
	def _constrains_payment_state(self):
		for rec in self.filtered(lambda x: x.move_type in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund') and x.payment_state == 'paid'):
			#Total de la factura en la moneda de la compañia
			total_invoice = rec.currency_id._convert(rec.amount_untaxed, rec.company_currency_id, rec.company_id, rec.currency_rate_ref)
			#Se obtienen las líneas de configuración que cumplan con las condiciones establecidas
			query_resul = rec._query_commission_conf_line(rec.department_id.id, rec.seller_employee_id.id, rec.assigned_employee_id.id, "'paid_bill'", total_invoice)
			#Se calcula el monto adicional de comisiones por producto
			commission_products = rec._calculate_commission_products()
			if (query_resul and commission_products):
				rec._create_paid_commission_line(query_resul, commission_products, total_invoice)
	...
	def _calculate_commission_products(self):
		products = self.invoice_line_ids.filtered(lambda line: line.product_id.product_commission)
		return sum([x.price_subtotal * (x.product_id.percentage) / 100 if x.product_id.commission_type == 'percentage' else x.product_id.fixed_amount for x in products])

	def _create_paid_commission_line(self, query_resul, commission_products, total_invoice):
		self.ensure_one()
		if query_resul:
			for cl in query_resul:
				total_commission = 0
				if cl.get('cs_id', False):
					total_commission = commission_products + cl.get('cs_fixed_amount') + (total_invoice * cl.get('cs_percentage') / 100)
				else:
					total_commission = commission_products + cl.get('ccl_amount') + (total_invoice * cl.get('global_percentage') / 100) 
					if cl.get('contract_employee') == self.seller_employee_id.id:
						total_commission += total_invoice * cl.get('direct_percentage') / 100
					elif cl.get('contract_employee') == self.assigned_employee_id.id:
						total_commission += total_invoice * cl.get('allocation_percentage') / 100
	...
	def _query_commission_conf_line(self, department, seller, assigned, calculation, total_invoice):
		self.ensure_one()
