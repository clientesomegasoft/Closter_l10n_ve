...
class EmployeeARCReport(models.TransientModel):
	...
	year = fields.Integer(string='Período', required=True, default=fields.Date.today().year)
	employee_ids = fields.Many2many('hr.employee', string='Empleados', compute='_compute_employee_ids', store=True, required=True, domain="[('company_id', '=', company_id)]")
	...
	def _compute_employee_ids(self):
		for rec in self:
			rec.employee_ids = [(5,)]
			if rec.type == 'company':
				rec.employee_ids = self.env['hr.employee'].search([('company_id', '=', rec.company_id.id)])
			elif rec.type == 'deparment' and rec.department_id:
				rec.employee_ids = self.env['hr.employee'].search([('company_id', '=', rec.company_id.id), ('department_id', '=', rec.department_id.id)])
	...
	def get_pdf(self):
		return self.env.ref('omegasoft_employee_arc.action_employee_arc_report').report_action(self)
	...
	def get_months_table(self):
		...
		for month in range(1,13):
			...
			month_name = format_date(self.env, date_from, date_format='MMMM').capitalize()
			params += [month_name, date_from, date_to]
		return self.env.cr.mogrify('(VALUES %s) AS months_table(month_name, date_from, date_to)' % ','.join(['(%s, %s, %s)'] * 12), params).decode(self.env.cr.connection.encoding)
	...
	def get_pdf_and_send(self):
		...
		if self.company_id.arc_template_id:
			...
			raise UserError(_("No existe plantilla para el envio de ARC configurada en ajuste de nomina."))
	# employee file code

	employee_file = fields.Boolean(
		'Employee file', related="company_id.employee_file")
