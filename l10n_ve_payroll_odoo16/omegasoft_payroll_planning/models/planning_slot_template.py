...
class PlanningSlotTemplate(models.Model):
    ...
    work_shifts_id = fields.Many2one(comodel_name='work.shifts', string='Shift name', ondelete='restrict')
    work_entry_type_id = fields.Many2one(comodel_name='hr.work.entry.type', string='Work entry type', ondelete='restrict')

    def name_get(self):
        result = []
        for shift_template in self:
            name = '%s - %s' % (
                shift_template.work_shifts_id.name if shift_template.work_shifts_id else shift_template.name,
