...
class HrLeave(models.Model):
    ...
    history_ids = fields.One2many(comodel_name='hr_plan_accumulation.history', inverse_name='leave_id', string="History", readonly=False, states={'confirm': [('readonly', True)], 'refuse': [('readonly', True)], 'validate': [('readonly', True)], 'vaidate1': [('readonly', True)], })
    ...
    @api.onchange('holiday_type', 'employee_ids', 'holiday_status_id', 'department_id', 'mode_company_id', 'category_id')
        ...
        if (self.is_plan_vacation or self.is_plan_enjoy) and self.holiday_status_id:
            ...
            if employees:
                ...
                for employee in employees:
                    accumulated_id = self.env['hr_plan_accumulation'].sudo().search([('employee_id', '=', employee._origin.id)])
                    if accumulated_id:
                        ...
                        for line in lines:
                            ...
                            if line.days_law > 0 or line.vacation_bonus > 0 or line.additional_days > 0:
        ...
        for leave in self:
            if leave.is_plan_vacation or leave.is_plan_enjoy:
                for history_line in leave.history_ids:
                    accumulated_id = leave.env['hr_plan_accumulation'].sudo().search([
                        ('employee_id', '=', history_line.employee_id.id)
                    ], limit=1)
                    ...
                    if accumulated_id:
                        ...
                        if line:
                            ...
                            vacation_bonus = line.vacation_bonus - history_line.vacation_bonus
                            additional_days = line.additional_days - history_line.additional_days
                            
                            if days_law >= 0 and vacation_bonus >= 0 and additional_days >= 0:
                                line.write({
                                    'days_law': days_law,
                                ...
                                raise UserError(_("Error: The legal days, vacation days, and additional days cannot be less than zero"))
        ...
        for record in self:
            if record.is_plan_vacation or record.is_plan_enjoy:
                if not record.history_ids:
                    raise UserError(_("This type of absence cannot be generated without their respective lines"))
                
    @api.constrains('request_date_from', 'request_date_to', 'liquidate')
    def _valid_number_day(self):
        if not self.liquidate and self.leave_type_request_unit == 'day' and (self.is_plan_vacation or self.is_plan_enjoy):
            days = 0
            for record in self.history_ids:
                days += record.days_law + record.additional_days
            if self.number_of_days_display != days:
                raise UserError(_("The number of days to be deducted does not match the number of days of the permission"))

    
    @api.model_create_multi
    def create(self, vals_list):
        for values in vals_list:
            if values.get('parent_id') and not 'history_ids' in values:
                leave_parent = self.env['hr.leave'].search([('id', '=', values.get('parent_id'))])
                if leave_parent.holiday_status_id.is_plan_vacation or leave_parent.holiday_status_id.is_plan_enjoy:
                    # Obtén las líneas de historial filtradas para el empleado en particular
                    history_lines = leave_parent.history_ids.filtered(lambda x: x.employee_id.id == values.get('employee_id'))
