...
class HrPlanAccumulationHistory(models.Model):
    ...
    request_date_from = fields.Date('Request Start Date', related="leave_id.request_date_from")
    request_date_to = fields.Date('Request End Date', related="leave_id.request_date_to")
    ...
    def _onchange_search_period(self):
        if self.employee_id and self.period:
            accumulated_id = self.env['hr_plan_accumulation'].sudo().search([('employee_id', '=', self.employee_id.id)])
            if self.leave_id.is_plan_vacation:
                line = self.env['hr_plan_accumulation.vacation'].search([('accumulated_id', '=', accumulated_id.id)]).filtered(lambda x: x.period.year == self.period.year)
            elif self.leave_id.is_plan_enjoy:
                line = self.env['hr_plan_accumulation.enjoy'].search([('accumulated_id', '=', accumulated_id.id)]).filtered(lambda x: x.period.year == self.period.year)
            ...
            if line:
                ...
                raise ValidationError(_("Do not have vacations for the selected period"))
