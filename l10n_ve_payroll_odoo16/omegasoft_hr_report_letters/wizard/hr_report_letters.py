...
class HrReportLetters(models.TransientModel):
    ...
    export_bank_payment_id = fields.Many2one('export.bank.payments', string='TXT Number')
    ...
    def button_print(self):
        if self.letter_type == 'work':
            ...
            return self.env.ref('omegasoft_hr_report_letters.report_hr_work_letter_action').report_action(None)
            
        elif self.letter_type == 'txt':
            if not self.export_bank_payment_id:
                raise ValidationError("Debe seleccionar un Número de TXT!")
            return self.env.ref('omegasoft_hr_report_letters.report_hr_txt_letter_action').report_action(None)
