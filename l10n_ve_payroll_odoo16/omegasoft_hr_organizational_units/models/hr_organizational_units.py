...
class HrOrganizatioanlUnits(models.Model):
    ...
    jobs_ids = fields.Many2many('hr.job', string='Jobs', domain="[('company_id', '=', company_id)]")
    ...
    employee_feedback_template = fields.Html(compute='_compute_appraisal_feedbacks', store=True, readonly=False)
    manager_feedback_template = fields.Html(compute='_compute_appraisal_feedbacks', store=True, readonly=False)
    ...
    def _compute_appraisals_to_process(self):
        appraisals = self.env['hr.appraisal'].read_group(
            [('organizational_units_id', 'in', self.ids), ('state', 'in', ['new', 'pending'])], ['organizational_units_id'], ['organizational_units_id'])
        result = dict((data['organizational_units_id'][0], data['organizational_units_id_count']) for data in appraisals)
        for units in self:
    ...
    def _compute_appraisal_feedbacks(self):
        for units in self:
            units.employee_feedback_template = units.company_id.appraisal_employee_feedback_template or self.env.company.appraisal_employee_feedback_template
            units.manager_feedback_template = units.company_id.appraisal_manager_feedback_template or self.env.company.appraisal_manager_feedback_template
    ...
    def _compute_plan_count(self):
        plans_data = self.env['hr.plan']._read_group([('organizational_units_id', 'in', self.ids)], ['organizational_units_id'], ['organizational_units_id'])
        plans_count = {x['organizational_units_id'][0]: x['organizational_units_id_count'] for x in plans_data}
        for unit in self:
    ...
    member_ids = fields.One2many('hr.employee', 'organizational_units_id', string='Members', readonly=True)
    
    def _compute_total_employee(self):
        emp_data = self.env['hr.employee']._read_group([('organizational_units_id', 'in', self.ids)], ['organizational_units_id'], ['organizational_units_id'])
        result = dict((data['organizational_units_id'][0], data['organizational_units_id_count']) for data in emp_data)
        for units in self:
