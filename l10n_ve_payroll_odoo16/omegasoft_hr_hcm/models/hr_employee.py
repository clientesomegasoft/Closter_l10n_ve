...
class HrEmployee(models.Model):
    ...
    hcm_insurer_id = fields.Many2one(
        'hr.hcm.insurer',
        string = 'Insurer')
    ...
    hcm_coverage_scale_id = fields.Many2one(
        'hr.hcm.coverage.scale',
        string = 'HCM coverage scale')

    hcm_total_quota_amount = fields.Monetary('Total quota amount', currency_field='hcm_currency_id')
    hcm_total_policy_coverage = fields.Monetary('Total policy coverage', currency_field='hcm_currency_id')
    hcm_company_quota_amount = fields.Monetary('Company quota contribution', currency_field='hcm_currency_id')
    hcm_employee_quota_amount = fields.Monetary('Employee quota contribution', currency_field='hcm_currency_id')
