...
class AccountMove(models.Model):
	...
	withholding_municipal_ids = fields.One2many('account.withholding.municipal', 'invoice_id', string='Retenciones de municipales', readonly=True, domain=[('state', '!=', 'cancel')])
	municipal_concept_ids = fields.Many2many('account.municipal.concept', string='Conceptos de retención municipal', readonly=True, states={'draft': [('readonly', False)]}, copy=False)
	apply_withholding_municipal = fields.Boolean(compute='_apply_withholding_municipal')

	@api.depends('partner_id', 'move_type')
	def _apply_withholding_municipal(self):
		for move in self:
			move.apply_withholding_municipal = (
				move.is_sale_document() and move.partner_id.is_municipal_agent
			) or (
				move.is_purchase_document() and move.company_id.partner_id.is_municipal_agent
			)
	...
	def _post(self, soft=True):
		...
		for move in to_post:
			if move.apply_withholding_municipal and move.municipal_concept_ids:
				...
				base_amount = abs(self.company_id.fiscal_currency_id.id == self.company_currency_id.id and self.amount_untaxed_signed or self.amount_untaxed_ref)
				...
				for concept_id in move.municipal_concept_ids:
					...
					withholding_id = move.withholding_municipal_ids.filtered(lambda w: w.line_ids.municipal_concept_id.id == concept_id.id and w.state == 'draft')
					if withholding_id:
						values['line_ids'] = [Command.update(withholding_id.line_ids.id, line_ids)]
						withholding_id.write(values)
					else:
						values['line_ids'] = [Command.create(line_ids)]
						withholding_id = self.env['account.withholding.municipal'].create(values)
	...
	def button_draft(self):
		super().button_draft()
		self.mapped('withholding_municipal_ids').filtered(lambda w: w.state == 'posted').button_draft()

	def button_cancel(self):
		super().button_cancel()
		self.mapped('withholding_municipal_ids').filtered(lambda w: w.state == 'draft').button_cancel()
