...
class FiscalBooksReportHandler(models.AbstractModel):
	...
	def _custom_options_initializer(self, report, options, previous_options=None):
		...
		options['buttons'] += [
			{'name': 'Generar libro de ventas', 'action': 'datailed_xlsx', 'action_param': 'sale', 'sequence': 90},
			{'name': 'Generar libro de compras', 'action': 'datailed_xlsx', 'action_param': 'purchase', 'sequence': 90}
		]

	def _caret_options_initializer(self):
		return {
			'account.move.line': [{'name': 'Ver apuntes contables', 'action': 'caret_option_account_move_line'}],
			'account.withholding.iva': [{'name': 'Ver retenciones', 'action': 'caret_option_account_withholding_iva'}],
		}

	def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
		def _get_section_tax_lines(domain):
			balance, tax_base_amount = self.env.company.currency_id.id == fiscal_currency_id.id and ('balance', 'tax_base_amount') or ('balance_ref', 'tax_base_amount_ref')
		...
		def _get_section_withholding_line(withholding_type):
			...
			''', [withholding_type, options['date']['date_from'], options['date']['date_to'], tuple(options.get('multi_company', self.env.company.ids))])
		...
		for id, section in SECTIONS.items():
			section_tax_lines = _get_section_tax_lines(domain = [('move_id.move_type', 'in', section['move_type']), ('move_id.not_in_fiscal_book', '=', False)])
			...
			lines.append((0, {
				...
				'columns': [{
					'no_format': value,
					'name': report.format_value(value, figure_type='monetary', currency=fiscal_currency_id),
					'style': 'white-space:nowrap;'
				} for value in [sum(i) for i in zip(*section_tax_lines.values())]]
			}))

			#TAX LINES
			for t_type, name in section['lines'].items():
				lines.append((0, {
					'id': report._get_generic_line_id(None, None, markup=t_type, parent_line_id=section_id),
					'name': name,
					'caret_options': 'account.move.line' if section_tax_lines.get(t_type) else None,
					'unfoldable': False,
			...
			#WITHHOLDING LINE
			withholding_line = _get_section_withholding_line(section['withholding']['type'])
			lines.append((0, {
				'id': report._get_generic_line_id(None, None, markup=section['withholding']['type']),
				'name': section['withholding']['name'],
				'caret_options': 'account.withholding.iva' if withholding_line[1] else None,
				'level': 2,
				'columns': [{
					'no_format': value,
					'name': report.format_value(value, figure_type='monetary', currency=fiscal_currency_id),
	...
	def caret_option_account_move_line(self, options, params):
		...
		section, t_type = [report._get_markup(line_id) for line_id in params['line_id'].split('|')[-2:]]
		domain = report._get_options_domain(options, 'strict_range')
		domain += [('move_id.move_type', 'in', SECTIONS[section]['move_type']), ('move_id.not_in_fiscal_book', '=', False)]

		if t_type == 'exempt':
			domain += [('display_type', '=', 'product'), ('tax_ids.fiscal_tax_type', '=', t_type)]
		else:
			domain += [('display_type', '=', 'tax'), ('tax_line_id.fiscal_tax_type', '=', t_type)]
	...
	def caret_option_account_withholding_iva(self, options, params):
		...
		return {
			...
			'views': [[self.env.ref('l10n_ve_withholding_iva.account_withholding_iva_view_tree').id, 'list'], [False, 'form']],
			'domain': [
				('state', '=', 'posted'),
				...
				('company_id', 'in', options.get('multi_company', self.env.company.ids)),
			],
		}
	...
	def get_headers(self, options):
		...
		return [[
			...
			{'name': 'Nombre ó razón social', 'key': 'partner_name', 'set_column': 30},
			...
			{'name': f'{name} exentas o no gravadas', 'key': 'base_exempt', 'sum': 0.0},
			{'name': 'Base imponible alícuota general', 'key': 'base_general', 'sum': 0.0},
			{'name': '% alícuota general', 'key': 'tax_general'},
			{'name': 'Impuesto alícuota general', 'key': 'amount_general', 'sum': 0.0},
			{'name': 'Base imponible alícuota reducida', 'key': 'base_reduced', 'sum': 0.0},
			{'name': '% alícuota reducida', 'key': 'tax_reduced'},
			{'name': 'Impuesto alícuota reducida', 'key': 'amount_reduced', 'sum': 0.0},
			{'name': 'Base imponible alícuota adicional', 'key': 'base_additional', 'sum': 0.0},
			{'name': '% alícuota adicional', 'key': 'tax_additional'},
			{'name': 'Impuesto alícuota adicional', 'key': 'amount_additional', 'sum': 0.0},
	...
	def _select(self, options):
		amount_total, tax_base_amount, balance = self.env.company.currency_id.id == self.env.company.fiscal_currency_id.id and \
			('amount_total_signed', 'tax_base_amount', 'balance') or ('amount_total_ref', 'tax_base_amount_ref', 'balance_ref')
		...
		return f'''
			...
			SUM(CASE
				WHEN account_move.state = 'posted' AND account_move.invoice_date >= %(date_from)s AND account_move_line.display_type = 'tax' AND account_tax.fiscal_tax_type = %({t})s THEN
					ABS(account_move_line.{balance}) * CASE WHEN account_move.move_type IN ('out_refund', 'in_refund') THEN -1 ELSE 1 END
				ELSE 0.0
			END) AS amount_{t}''' for t in ('general', 'reduced', 'additional')) + ''',
	...
	def get_xlsx(self, options, response=None):
		...
		header_style_1 = workbook.add_format({'font_name': 'Helvetica Neue', 'bold': True, 'font_size': 8, 'align': 'center', 'valign': 'vcenter', 'bg_color': '#00b050', 'border': True})
		header_style_2 = workbook.add_format({'font_name': 'Helvetica Neue', 'bold': True, 'font_size': 8, 'align': 'center', 'valign': 'vcenter', 'bg_color': '#b4c6e7', 'border': True})
		default_style = workbook.add_format({'font_name': 'Helvetica Neue', 'font_size': 8, 'border': True})
		date_style = workbook.add_format({'font_name': 'Helvetica Neue', 'font_size': 8, 'border': True, 'num_format': 'yyyy-mm-dd'})
		total_style = workbook.add_format({'font_name': 'Helvetica Neue', 'bold': True, 'font_size': 8, 'bg_color': '#aeabab', 'border': True})
			...
			for column in header:
				...
				if column['name']:
					if colspan == 1:
						...
						sheet.merge_range(y_offset, x_offset, y_offset, x_offset + colspan - 1, column['name'], header_style_1)
				x_offset += colspan
			y_offset += 1
		...
		y_offset += 2
		sheet.merge_range(y_offset, 1, y_offset, 2, 'RESUMEN DE %s' % report_name.upper(), header_style_1)
		...
		y_offset += 1
		sheet.merge_range(y_offset, 1, y_offset, 2, 'Total %s y débitos/créditos fiscales' % SECTIONS[options['file_type']]['name'].lower(), default_style)
