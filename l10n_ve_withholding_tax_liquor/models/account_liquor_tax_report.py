...
class AccountLiquourTaxReport(models.Model):
	...
	state = fields.Selection([('draft', 'Borrador'), ('confirmed', 'Confirmado'), ('paid', 'Pagado')], string='Estado', default='draft', required=True, readonly=True, copy=False)
	...
	line_ids = fields.One2many('account.liquor.tax.report.line', 'report_id', string='Lineas', readonly=True)
	...
	def seek_for_lines(self):
		...
		for tax_id, rate, line_ids in rows:
			...
			for l in move_line_ids:
				price_subtotal = l.company_currency_id.round(l.price_subtotal / l.currency_rate)
				if not is_company_currency:
					price_subtotal = l.currency_ref_id.round(price_subtotal * l.currency_rate_ref.rate)
...
class AccountLiquourTaxReportLine(models.Model):
	...
	report_id = fields.Many2one('account.liquor.tax.report', required=True, ondelete='cascade')
	liquor_tax_id = fields.Many2one('account.liquor.tax', string='Impuesto', required=True)
	...
	_sql_constraints = [('unique_liquor_tax_per_report', 'UNIQUE(report_id, liquor_tax_id)', 'Solo se puede tener una linea por tipo de impuesto')]
