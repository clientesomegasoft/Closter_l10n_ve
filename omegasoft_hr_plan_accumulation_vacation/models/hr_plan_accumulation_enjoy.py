...
class HrPlanAccumulationEnjoy(models.Model):
    ...
    period = fields.Date(
        string="Period",
        required=True,
        help="""It refers to the year to which the vacation days belong,
        Field will be increased automatically by the planned action.""",
    )
    days_law = fields.Float(
        string="Day Law",
        help="""It refers to the 15 fixed legal days that are increased
        by the planned action or by adding lines.""",
    )
    vacation_bonus = fields.Float(
        string="Vacation bonus",
        help="""It refers to the fixed 15-day Vacation Bonus that is
        increased by the planned action or by adding lines.""",
    )
    additional_days = fields.Float(
        string="Additional Days",
        help="""This calculation of the field of additional days depends
        on the seniority in years, it is one day for each year of services
        from the second year with a cap of 15 days""",
    )
