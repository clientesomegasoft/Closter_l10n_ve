...
class AccountReport(models.Model):
    ...
    filter_currency = fields.Boolean(
        string="Currency",
        compute=lambda x: x._compute_report_option_filter('filter_currency', True), readonly=False, store=True, depends=['root_report_id'],
    )

    def _init_options_currency(self, options, previous_options=None):
        if self.filter_currency:
            options['selected_currency'] = previous_options and previous_options.get('selected_currency') or self.env.company.currency_id.id
            options['currencies'] = previous_options and previous_options.get('currencies') or [{'id': c.id, 'name': c.name} for c in (self.env.company.currency_id, self.env.company.currency_ref_id)]
        ...
        if options.get('selected_currency', 0) == self.env.company.currency_ref_id.id:
            ...
            for line in lines:
                for column in line['columns']:
                    if column.get('name') and type(column.get('no_format')) in (int, float) and self.env.company.currency_id.symbol in column['name']:
                        column['name'] = self.format_value(column['no_format'], currency=currency, figure_type='monetary')
        return lines
