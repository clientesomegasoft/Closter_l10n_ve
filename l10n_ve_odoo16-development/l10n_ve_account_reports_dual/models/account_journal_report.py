    ...
    def _get_generic_tax_summary_for_sections(self, options, data):
        ...
        for tax in taxes:
            res[tax.country_id.name].append({
                'base_amount': report.format_value(tax_values[tax.id]['base_amount'], blank_if_zero=False, currency=self.env['res.currency'].browse(options.get('selected_currency')), figure_type='monetary'),
                'tax_amount': report.format_value(tax_values[tax.id]['tax_amount'], blank_if_zero=False, currency=self.env['res.currency'].browse(options.get('selected_currency')), figure_type='monetary'),
                'name': tax.name,
