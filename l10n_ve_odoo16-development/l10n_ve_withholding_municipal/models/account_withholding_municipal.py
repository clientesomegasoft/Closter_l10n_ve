...
class AccountWithholdingMunicipal(models.Model):
	...
	type = fields.Selection([('customer', 'Retención municipal clientes'), ('supplier', 'Retención municipal proveedores')], required=True, readonly=True)
	...
	line_ids = fields.One2many('account.withholding.municipal.line', 'withholding_municipal_id', string='Lineas', readonly=True)
	...
	def button_post(self):
		...
		if self.type == 'supplier' and (not self.name or self.name == 'NUMERO DE COMPROBANTE'):
			self.name = self.date.strftime('%Y%m') + self.env['ir.sequence'].with_company(self.company_id).next_by_code('account_withholding_municipal')
	...
	def button_send_mail(self):
		self.ensure_one()
		template_id = self.env.ref('l10n_ve_withholding_municipal.mail_template_withholding_municipal_receipt').id
		return {
...
class AccountWithholdingMunicipalLine(models.Model):
	...
	withholding_municipal_id = fields.Many2one('account.withholding.municipal', ondelete='cascade', required=True)
	municipal_concept_id = fields.Many2one('account.municipal.concept', string='Concepto de retención municipal', required=True)
	...
	currency_id = fields.Many2one('res.currency', related='company_id.fiscal_currency_id', store=True)
	company_id = fields.Many2one('res.company', related='withholding_municipal_id.company_id', store=True)