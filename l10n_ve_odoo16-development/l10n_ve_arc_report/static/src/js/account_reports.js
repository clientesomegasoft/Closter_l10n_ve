...
accountReportsWidget.include({
	...
	render_searchview_buttons: function() {
		...
		//YEAR FILTER
		var $yearpickers = this.$searchview_buttons.find('.js_account_reports_yearpicker');
		...
		this.$searchview_buttons.find('.js_account_report_year_filter').click(function (e) {
			var year = self.$searchview_buttons.find('.o_datepicker_input[name="year"]');
			var error = year.val() === "";
		...
		if ('partner_id' in this.report_options) {
			if (!this.partner_m2o_filter) {
				...
				this.partner_m2o_filter.appendTo(this.$searchview_buttons.find('.js_m2o_filters'));
			} else {
				this.$searchview_buttons.find('.js_m2o_filters').append(this.partner_m2o_filter.$el);
			}
		}
	}
});
