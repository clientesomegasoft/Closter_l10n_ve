...
class ReportArc(models.AbstractModel):
	...
	def _get_months_table(self, options):
		months_table = '(VALUES %s) AS months_table(date_from, date_to)' % ','.join(['(%s, %s)'] * 12)
		params = []
		for month in range(1,13):
			date_from = fields.Date.from_string('%s-%s-01' % (options['year'], month))
			date_to = date_from + relativedelta(day=31)
			params += [date_from, date_to]
		return self.env.cr.mogrify(months_table, params).decode(self.env.cr.connection.encoding)
	...
	def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
		...
		self._cr.execute("""
			...
			ORDER BY month
		""".format(
			amount_total = self.env.company.currency_id == fiscal_currency_id and 'amount_total_signed' or 'amount_total_ref',
		...
		for month in self._cr.dictfetchall():
			...
			lines.append((0, {
				'id': report._get_generic_line_id(None, None, markup=month['month']),
				'name': format_date(self.env, month['month'], date_format='MMMM').capitalize(),
				'caret_options': 'account.withholding.islr',
				'columns': [
					{'name': report.format_value(month['total_invoice'], currency=fiscal_currency_id, figure_type='monetary')},
					{'name': report.format_value(month['base'], currency=fiscal_currency_id, figure_type='monetary')},
					{'name': report.format_value(month['amount'], currency=fiscal_currency_id, figure_type='monetary')},
					{'name': report.format_value(sum_base, currency=fiscal_currency_id, figure_type='monetary')},
					{'name': report.format_value(sum_amount, currency=fiscal_currency_id, figure_type='monetary')},
				],
			}))
		lines.append((0, {
			...
			'level': 1,
			'columns': [
				{'name': report.format_value(sum_total_invoice, currency=fiscal_currency_id)},
				{'name': report.format_value(sum_base, currency=fiscal_currency_id)},
				{'name': report.format_value(sum_amount, currency=fiscal_currency_id)},
			],
		}))
		return lines

	def _caret_options_initializer(self):
		return {'account.withholding.islr': [{'name': 'Ver retenciones', 'action': 'view_withholding_entries'}]}
	...
	def view_withholding_entries(self, options, params):
		...
		return {
			...
			'views': [[self.env.ref('l10n_ve_withholding_islr.account_withholding_islr_view_tree').id, 'list'], [False, 'form']],
			'domain': [
				('date', '>=', date_from),
