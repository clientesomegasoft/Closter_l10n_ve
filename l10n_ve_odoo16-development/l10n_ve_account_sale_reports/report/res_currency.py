...
class ResCurrency(models.Model):
    ...
    def _get_query_currency_ref_table(self, options):
        ...
        if options.get('multi_company', False):
            ...
            currency_rates = companies.mapped('currency_ref_id')._get_rates(user_company, conversion_date)
        else:
        ...
        query = '(VALUES %s) AS operative_currency_table(company_id, rate, precision)' % ','.join('(%s, %s, %s)' for i in companies)
