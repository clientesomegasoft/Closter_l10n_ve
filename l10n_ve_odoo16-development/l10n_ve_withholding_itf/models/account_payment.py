...
class AccountPayment(models.Model):
	...
	calculate_itf = fields.Boolean(string='Permitir ITF', default=True, readonly=True, states={'draft': [('readonly', False)]})
	...
	def action_post(self):
		...
		if not (self.apply_itf and self.calculate_itf) or self.currency_id.is_zero(amount_currency):
			if self.itf_move_id:
		...
		balance = amount_currency if self.currency_id == self.company_currency_id else self.company_currency_id.round(amount_currency / self.currency_rate_ref.rate)
		move_line_values = [{
