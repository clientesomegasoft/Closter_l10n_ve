...
class AccountMove(models.Model):
	...
	withholding_iva_id = fields.Many2one('account.withholding.iva', string='Retencion de IVA', readonly=True, copy=False)
		...
		for move in to_post:
			if move._apply_withholding_iva():
				...
				if move.withholding_iva_id:
					...
					move.withholding_iva_id = self.env['account.withholding.iva'].create(values)
	...
	def _apply_withholding_iva(self):
		self.ensure_one()
		return self.partner_id.person_type_code in ('PJDO', 'PNRE') and self.line_ids.tax_line_id and ((
			self.is_sale_document() and self.partner_id.is_iva_agent and self.company_id.partner_id.iva_rate_id
		) or (
			self.is_purchase_document() and self.company_id.partner_id.is_iva_agent and self.partner_id.iva_rate_id
		))
	...
	def _get_withholding_iva_properties(self):
		...
		tax_base_amount, balance = self.company_id.fiscal_currency_id.id == self.company_currency_id.id and\
		('tax_base_amount', 'balance') or ('tax_base_amount_ref', 'balance_ref')

		for line in self.line_ids:
			if line.display_type == 'product' and line.price_total == line.price_subtotal:
				...
				line_ids.append(Command.create({
					...
					'base_amount_currency': abs(line.amount_currency), #in invoice currency
					'base_amount_company_currency': abs(line.balance), #in company currency
					'base_amount_currency_ref': abs(line.balance_ref), #in referencial currency
				}))
