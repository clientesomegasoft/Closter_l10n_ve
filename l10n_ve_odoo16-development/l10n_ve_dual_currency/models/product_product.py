...
class ProductProduct(models.Model):
	...
	standard_price_ref = fields.Float(string='Coste op.', company_dependent=True, digits='Product Price')
	value_svl_ref = fields.Float(compute='_compute_value_svl', compute_sudo=True)
	avg_cost_ref = fields.Monetary(string='Costo unitario op.', compute='_compute_value_svl', compute_sudo=True, currency_field='cost_currency_ref_id')
	total_value_ref = fields.Monetary(string='Valor total op.', compute='_compute_value_svl', compute_sudo=True, currency_field='cost_currency_ref_id')

	def write(self, vals):
		if 'standard_price' in vals and 'standard_price_ref' not in vals:
			vals['standard_price_ref'] = vals['standard_price'] * self.cost_currency_ref_id.rate
		elif 'standard_price_ref' in vals and 'standard_price' not in vals:
			vals['standard_price'] = vals['standard_price_ref'] / self.cost_currency_ref_id.rate
	...
	def _compute_value_svl(self):
		...
		groups = self.env['stock.valuation.layer']._read_group(domain, ['value:sum', 'value_ref:sum', 'quantity:sum'], ['product_id'])
		...
		for group in groups:
			...
			if not avg_cost and product.sudo(False).qty_available > 0:
				avg_cost = product.standard_price_ref / self.env.company.currency_ref_id.rate
			if not avg_cost_ref and product.sudo(False).qty_available > 0:
				avg_cost_ref = product.standard_price * self.env.company.currency_ref_id.rate
	...
	def _prepare_in_svl_vals(self, quantity, unit_cost):
		company = self.env['res.company'].browse(self.env.context.get('force_company', self.env.company.id))
		vals = super()._prepare_in_svl_vals(quantity, unit_cost)
		value_ref = company.currency_ref_id.round(unit_cost * quantity * company.currency_ref_id.rate)
		vals.update({
			'value_ref': value_ref,
			'unit_cost_ref': company.currency_ref_id.round(unit_cost * company.currency_ref_id.rate),
	...
	def _prepare_out_svl_vals(self, quantity, company):
		...
		vals = {
			...
			'value_ref': company.currency_ref_id.round(quantity * self.standard_price_ref),
	...
	def _change_standard_price(self, new_price):
		...
		rounded_new_price_ref = float_round(new_price * company_id.currency_ref_id.rate, precision_digits=price_unit_prec)
		...
		for product in self:
			...
			value = company_id.currency_id.round((rounded_new_price * quantity_svl) - product.sudo().value_svl)
			value_ref = company_id.currency_ref_id.round((rounded_new_price_ref * quantity_svl) - product.sudo().value_svl_ref)

			if company_id.currency_id.is_zero(value) and company_id.currency_ref_id.is_zero(value_ref):
				continue
			elif company_id.currency_id.is_zero(value):
				description = _('Product value manually modified (from %s to %s)') % (product.standard_price_ref, rounded_new_price_ref)
			else:
		...
		account_moves = self.env['account.move'].sudo().with_context(skip_compute_balance_ref=True).create(am_vals_list)
		if account_moves:
			account_moves._post()
	...
	def _run_fifo(self, quantity, company):
		...
		for candidate in candidates:
			...
			candidate_unit_cost_ref = candidate.remaining_value_ref / candidate.remaining_qty
			new_standard_price = candidate_unit_cost
			new_standard_price_ref = candidate_unit_cost_ref
			value_taken_on_candidate = qty_taken_on_candidate * candidate_unit_cost
			value_taken_on_candidate = candidate.currency_id.round(value_taken_on_candidate)
			value_taken_on_candidate_ref = candidate.currency_ref_id.round(qty_taken_on_candidate * candidate_unit_cost_ref)
			new_remaining_value = candidate.remaining_value - value_taken_on_candidate
			new_remaining_value_ref = candidate.remaining_value_ref - value_taken_on_candidate_ref
			...
			if float_is_zero(qty_to_take_on_candidates, precision_rounding=self.uom_id.rounding):
				if float_is_zero(candidate.remaining_qty, precision_rounding=self.uom_id.rounding):
					...
					new_standard_price_ref = next_candidates and next_candidates[0].unit_cost_ref or new_standard_price_ref
				break
		...
		if values:
			self.sudo().with_company(company.id).with_context(disable_auto_svl=True).write(values)
