...
class ProductTemplate(models.Model):
	...
	list_price_ref = fields.Float(string='Precio de venta op.', digits='Product Price', compute='_compute_list_price_ref', inverse='_inverse_list_price_ref', store=True)
	...
	def _compute_list_price_ref(self):
		for template in self:
			template.list_price_ref = template.list_price * template.currency_ref_id.rate

	@api.onchange('list_price_ref')
	def _inverse_list_price_ref(self):
		for template in self:
			template.list_price = template.list_price_ref / template.currency_ref_id.rate
