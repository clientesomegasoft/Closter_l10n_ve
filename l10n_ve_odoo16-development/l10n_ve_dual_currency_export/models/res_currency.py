...
def to_utc(value, origin_tz = 'UTC'):
    return (origin_tz and pytz.timezone(origin_tz).localize(value).astimezone(pytz.UTC) or value).replace(tzinfo=None)
...
class CurrencyRate(models.Model):
    ...
    def _search_inverse_company_rate(self, operator, value):
        ...
        if operator in ('=', '!='):
            domain = ['&', ('rate','>',value - epsilon), ('rate','<',value + epsilon)]
            if operator == '!=':
                domain.insert(0, '!')
        else:
