...
class GrossIncomeReport(models.Model):
	...
	date_from = fields.Date(string='Desde', required=True, readonly=True, states={'draft': [('readonly', False)]})
	date_to = fields.Date(string='Hasta', required=True, readonly=True, states={'draft': [('readonly', False)]})
	town_hall_id = fields.Many2one('res.town.hall', string='Alcaldía', default=lambda self: self.env.company.town_hall_id, required=True, readonly=True)
	percentage = fields.Float(related='town_hall_id.percentage', store=True)
	line_ids = fields.One2many('gross.income.report.line', 'report_id', string='Lineas', readonly=True)
	amount_total = fields.Monetary(string='Total', compute='_compute_amount', store=True)
	amount_tax = fields.Monetary(string='Monto a declarar', compute='_compute_amount', store=True)
	move_id = fields.Many2one('account.move', string='Asiento', readonly=True)
	currency_id = fields.Many2one(related='company_id.fiscal_currency_id', string='Moneda', store=True)
	...
	def seek_for_lines(self):
		balance = self.currency_id.id == self.company_id.currency_id.id and 'balance' or 'balance_ref'
	...
	def button_post(self):
		...
		balance = self.currency_id._convert(amount_currency, self.company_id.currency_id, self.company_id, date)
		line_ids = [{
			'name': self.name,
		...
		expence_account_id = self.move_id.line_ids.filtered(lambda l: l.account_id.id == self.town_hall_id.expence_account_id.id)
		payable_account_id = self.move_id.line_ids.filtered(lambda l: l.account_id.id == self.town_hall_id.payable_account_id.id)
		line_ids_commands = [
			Command.update(expence_account_id.id, line_ids[0]) if expence_account_id else Command.create(line_ids[0]),
			Command.update(payable_account_id.id, line_ids[1]) if payable_account_id else Command.create(line_ids[1])
		]
		move_values = {
			'move_type': 'entry',
			'ref': self.name,
...
class GrossIncomeReportLine(models.Model):
	...
	report_id = fields.Many2one('gross.income.report', required=True, ondelete='cascade')
