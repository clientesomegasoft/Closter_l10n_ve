...
class HrEmployeeBonus(models.Model):
    ...
    name = fields.Selection(
        string='Type',
        selection=[('toy_voucher', 'Toy voucher'), ('christmas_bonus', 'Christmas Bonus'), ('school_voucher', 'School voucher'),
                ('birth_bonus', 'Birth Bonus')], tracking=True)

    active = fields.Boolean(
        default=True, help="Set active to false to hide the children tag without removing it.")

    description = fields.Char(
        string='Description', help='specific reference to the bonus payment.', tracking=True)

    state = fields.Selection(
        string='State',
        selection=[('new', 'New'), ('draft', 'Draft'),
                ('assigned', 'Assigned'), ('cancel', 'Rejected')],
    ...
    study_level = fields.Selection(string='Study level',
        selection=[('maternal','Maternal'),('elementary_school', 'Elementary school'), ('high_school', 'High School'), ('university', 'University')],
        tracking=True
    )
    ...
    rate_id = fields.Many2one('res.currency.rate', string='Rate', tracking=True, default=lambda self: self.env.company.currency_ref_id.rate_ids.filtered(lambda aml: aml.is_payroll_rate).sorted('name')[-1], domain="[('is_payroll_rate', '=', True)]")
    ...
    def _constrain_maximum_age(self):
        if self.maximum_age <= 0 and self.name in ['toy_voucher','birth_bonus']:
            raise ValidationError('La Edad máxima debe ser mayor a cero.')
    ...
    def _onchange_maximum_age(self):
        for record in self:
            ...
            if aux_maximum_age < aux_minimum_age:
                raise ValidationError("La edad máxima no debe ser inferior a la edad mínima.")
    ...
    def _search_employees(self, is_schooll_voucher=False):
        self._clean_lines()
        employee_obj = self.env['hr.employee'].search([]).filtered(lambda x: x.contract_id.state in ['open'])
        employee_list = []
        bonus_lines = []
        if self.name in ['toy_voucher', 'school_voucher', 'birth_bonus']:
            ...
            for employee in employee_obj.filtered(lambda x: x.family_information_ids):
                for child in employee.family_information_ids.filtered(lambda x: x.relationship in ['son','daughter']):
                    ...
                    if not is_schooll_voucher:
                        ...
                        children_list.append(
                            aux_child_age >= aux_minimum_age and aux_child_age <= aux_maximum_age)
                    else:
                        # School voucher
                ...
                if any(children_list):
                    bonus_lines.append({
                        ...
                        'employee_bonus_amount': children_list.count(True) * self.currency_id._convert(self.amount,self.env.company.currency_id,self.env.company,self.rate_id.name or fields.Date.today()) if self.rate_id.name else self.amount,
                    })
            ...
            for employee in employee_obj:
                bonus_lines.append({
                    'bonus_id': self.id,
                    'employee_id': employee.id,
                    'employee_bonus_amount': self.currency_id._convert(self.amount,self.env.company.currency_id,self.env.company,self.rate_id.name or fields.Date.today()) if self.rate_id.name else self.amount,
                })
