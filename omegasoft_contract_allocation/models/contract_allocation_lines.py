...
class HrContractAllocationLine(models.Model):
    ...
    quantity_to_deliver = fields.Float(string="Quantity to deliver", compute='_compute_allocated_quantity', store=True)
    ...
    def _onchange_allocated_quantity(self):
        for record in self:
            if record.allocated_quantity < record.quantity_delivered:
                raise UserError ('La cantidad asignada no puede ser menor que de la cantidad entregada.')
    ...
    def _onchange_quantity_delivered(self):
        for record in self:
            if record.quantity_delivered > record.allocated_quantity:
                ...
                record.quantity_to_deliver = record.allocated_quantity - record.quantity_delivered
            else:
    ...
    def _compute_allocated_quantity(self):
        for record in self:
            if record.allocated_quantity and record.quantity_delivered and record.date_delivered:
                record.quantity_to_deliver = record.allocated_quantity - record.quantity_delivered
            else:
                record.quantity_to_deliver = record.allocated_quantity
    ...
    def _compute_is_delivered(self):
        for record in self:
            record.is_delivered = True if record.quantity_to_deliver == 0 and record.date_delivered else False
    ...
    employee_file = fields.Boolean('Employee file', related="allocation_id.company_id.employee_file")
