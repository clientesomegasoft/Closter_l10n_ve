...
class HrContractAllocation(models.Model):
    ...
    name = fields.Selection(selection=[('employee', 'Employee')], string='Type of endowment',
    help='Indicates the type of endowment to be generated', tracking=True,)
    
    employee_ids = fields.Many2many(
        string='Employees', comodel_name='hr.employee', ondelete='restrict', domain= lambda self: self.env['hr.employee']._get_employee_l10n_ve_payroll_domain())
    ...
    department_ids = fields.Many2many(
        comodel_name='hr.department', ondelete='restrict', string='Departments', tracking=True,)
    ...
    def _check_employee_ids(self):
        if self.name == 'employee' and len(self.employee_ids) > 1:
            raise ValidationError("Las dotaciones por empleado sólo aceptan un máximo de 1 empleado")
    ...
    def _onchange_employee_ids(self):
        if self.name == 'employee':
            ...
            if self.employee_ids:
                self.job_title = self.employee_ids.job_title if self.name == 'employee' and len(self.employee_ids) == 1 else ' '
                self.write({'employee_contract_ids': [
    ...
    def _onchange_department_ids(self):
        if self.name == 'department':
            ...
            if self.department_ids:
                self.job_title = self.employee_ids.job_title if len(self.employee_ids) == 1 else ' '
                employees = self.get_employee_allocation_domain().filtered(lambda x: x.id in self.department_ids.member_ids.ids)
                if not employees:
    ...
    def get_lines_to_delivered(self):
        self.ensure_one()
        lines_tree_view = self.env.ref('omegasoft_contract_allocation.contract_allocation_lines_tree', raise_if_not_found=False)
        return {
            ...
            'domain': [('allocation_id.id', '=', self.id),('employee_id', 'in', self.employee_ids.ids)],
    ...
    employee_file_code_ids = fields.Many2many('employee.file.code', relation="employee_code_contract_allocation_rel", string='Employee File')
