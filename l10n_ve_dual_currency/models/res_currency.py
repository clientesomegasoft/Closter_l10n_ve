...
class CurrencyRate(models.Model):
	...
	def write(self, vals):
		if self.env['account.move.line'].search([('currency_rate_ref', '=', self.id)], limit=1) or self.env['stock.landed.cost.lines'].search([('currency_rate_ref', '=', self.id)], limit=1):
			raise UserError('No es posible ejecutar esta acción ya que la tasa está siendo utilizada por otro modelo.')
