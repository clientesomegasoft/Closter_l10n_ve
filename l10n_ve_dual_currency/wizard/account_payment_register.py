...
class AccountPaymentRegister(models.TransientModel):
	...
	currency_rate_ref = fields.Many2one('res.currency.rate', string='Tasa', compute='_compute_currency_rate_ref', store=True, readonly=False, domain="[('currency_id', '=', currency_ref_id)]")
	...
	def _get_total_amount_in_wizard_currency_to_full_reconcile(self, batch_result, early_payment_discount=True):
		...
		if self.source_currency_id == self.currency_id:
			...
			if self.currency_id == self.currency_ref_id:
				return abs(sum(self.currency_id.round(aml.amount_residual * aml.move_id.currency_rate_ref.rate) for aml in batch_result['lines'])), False
			else:
	...
	def _create_payment_vals_from_wizard(self, batch_result):
		...
		conversion_rate = self.currency_rate_ref.rate if self.currency_id == self.currency_ref_id else self.env['res.currency']._get_conversion_rate(
			self.currency_id,
