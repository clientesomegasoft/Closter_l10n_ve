...
class ExportBankPaymentsBanavih(models.Model):
    ...
    def action_done(self):
        ...
        if self.type_trans == 'fiscal':
            ...
            fiscal_code = fields.Date.today().strftime("%-d") + f'{self.date_end.month:0>2}' + str(self.date_end.year)
            banavih_code = self.env.company.banavih_code
            if not banavih_code:
                raise ValidationError("No se encuentra configurado un Código de Identificación Banavih!")
            data, filename = self._write_attachment(root, banavih_code + fiscal_code, False)
        else:
    ...
    def generate_fiscal_payroll(self):
        ...
        for employee in employees:
            ...
            employee_id = re.search('(\d+)', employee.identification_id).group() if re.search('(\d+)', employee.identification_id) else ''
