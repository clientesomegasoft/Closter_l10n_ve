...
class AccountReport(models.Model):
    ...
    filter_year = fields.Boolean(
        string="Año",
        compute=lambda x: x._compute_report_option_filter("filter_year"),
        readonly=False,
        store=True,
    ...
    filter_partner_id = fields.Boolean(
        string="Proveedor",
        compute=lambda x: x._compute_report_option_filter("filter_partner_id"),
        readonly=False,
        store=True,
        depends=["root_report_id"],
    )
    ...
    def _init_options_partner_id(self, options, previous_options=None):
        if self.filter_partner_id:
            options["partner_id"] = (
                previous_options and previous_options.get("partner_id") or False
            )
    ...
    def export_to_xlsx(self, options, response=None):
        ...
        default_border_style = workbook.add_format(
            {"font_name": "Arial", "align": "center", "border": 2}
        )
        title_style = workbook.add_format(
            {"font_name": "Arial", "bold": True, "border": 1, "bg_color": "#e7e6e6"}
        )
        bold_style = workbook.add_format(
            {"font_name": "Arial", "bold": True, "align": "center"}
        )
        border_bold_style = workbook.add_format(
            {"font_name": "Arial", "bold": True, "align": "center", "border": 2}
        )
        sign_style = workbook.add_format(
            {"font_name": "Arial", "bold": True, "align": "center", "top": 1}
        )
        ...
        if company_id.image_512:
            sheet.insert_image(
                ...
                "image.png",
                {
                    "x_scale": 0.2,
                    "y_scale": 0.2,
                    "image_data": io.BytesIO(base64.b64decode(company_id.image_512)),
                },
            )
        sheet.merge_range(
            2,
            ...
            2,
            6,
            "RELACION ANUAL DE LA RETENCION DE IMPUESTO SOBRE LA RENTA",
            bold_style,
        )
        sheet.merge_range(
            3,
            ...
            3,
            6,
            "DESDE: 01/01/{year}  HASTA: 31/12/{year}".format(year=options["year"]),
            bold_style,
        )
        ...
        sheet.merge_range(
            ...
            7,
            6,
            company_id.contact_address.replace("\n", " "),
        ...
        sheet.merge_range(
            ...
            12,
            6,
            partner_id.contact_address.replace("\n", " "),
            default_border_style,
        )
            ...
            sheet.insert_image(
                ...
                "image.png",
                {
                    ...
                    "image_data": io.BytesIO(
                        base64.b64decode(self.env.company.sign_512)
                    ),
                },
            )
        y_offset += 5
