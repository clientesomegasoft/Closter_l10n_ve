...
class ResConfigSettings(models.TransientModel):
    ...
    employee_file = fields.Boolean(
        "Employee File", related="company_id.employee_file", readonly=False
    )
    sequence_next_number = fields.Integer(
        "Sequence Next Number",
        related="company_id.sequence_next_number",
        readonly=False,
    )
...
class ResCompany(models.Model):
    ...
    def write(self, vals):
        ...
        sequence = self.env["ir.sequence"].search(
            [("code", "=", "hr.employee.code"), ("company_id", "=", self.id)], limit=1
        )
        file_code_access = self.env["ir.model.access"].search(
            [("name", "=", "omegasoft_hr_employee_code.employee_file_code")]
        )
