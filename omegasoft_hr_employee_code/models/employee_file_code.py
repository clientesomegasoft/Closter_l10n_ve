...
class EmployeeFileCode(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            "There cannot be two cards with the same name.",
        ),
        (
            "uniq_employee",
            "UNIQUE (employee_id, company_id)",
            "There cannot be two cards with the same employee.",
        ),
    ]
    ...
    def _set_contract_loans_allocation(self, pre_employee=None):
        ...
        if pre_employee:
            ...
            if self.valid_module_state(
                "omegasoft_hr_employee_advances_loans_discounts"
            ):
                pre_employee.advances_loans_discounts_line_ids._constrains_employee()
                self.env["hr_employee_advances_loans_discounts"].search(
                    [("employee_ids", "in", pre_employee.id)]
                )._constrains_employee()
            if self.valid_module_state("omegasoft_contract_allocation"):
                self.env["contract_allocation_lines"].search(
                    [("employee_id", "=", pre_employee.id)]
                )._constrains_employee()
                self.env["contract_allocation"].search(
