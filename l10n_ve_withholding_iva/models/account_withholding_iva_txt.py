...
class WithholdingIVATXT(models.Model):
	...
	line_ids = fields.One2many('account.withholding.iva', 'txt_id', string='Líneas', readonly=True, states={'draft': [('readonly', False)]})
	...
	total_amount = fields.Monetary(string='Monto a declarar', compute='_compute_amount', store=True)
	...
	def _compute_amount(self):
		for rec in self:
			...
			for line in rec.line_ids:
				...
				amount += line.amount
				total_amount += line.amount if line.invoice_id.move_type == 'in_invoice' else -line.amount
	...
	def unlink(self):
		for rec in self:
			if rec.state != 'cancel':
				raise UserError('Solo TXT en estado Cancelado pueden ser suprimidos.')
		return super().unlink()
		...
		for withholding_id in self.line_ids:
			...
			for line in withholding_id.line_ids:
				data.append('\t'.join((
					...
					invoice.nro_ctrl,
					format(line.tax_base_amount + line.tax_amount + exempt_amount),
