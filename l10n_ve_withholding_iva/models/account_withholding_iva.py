...
class AccountWithholdingIVA(models.Model):
    ...
    def button_post(self):
        ...
        if self.type == "supplier" and self.name == "NUMERO DE COMPROBANTE":
            self.name = self.date.strftime("%Y%m") + self.env[
                "ir.sequence"
            ].with_company(self.company_id).next_by_code("account_withholding_iva")
    ...
    def button_send_mail(self):
        ...
        template_id = self.env.ref(
            "l10n_ve_withholding_iva.mail_template_withholding_iva_receipt"
        ).id
        return {
    ...
    def _prepare_move_values(self):
        ...
        for line in self.line_ids:
            amount_currency += self.invoice_id.currency_id.round(
                line.base_amount_currency * iva_rate * sign
            )
            balance += self.company_id.currency_id.round(
                line.base_amount_company_currency * iva_rate * sign
            )
            balance_ref += self.company_id.currency_ref_id.round(
                line.base_amount_currency_ref * iva_rate * sign
            )
...
class AccountWithholdingIVALine(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", related="withholding_iva_id.company_id", store=True
    )
    ...
    def _compute_amount(self):
        for line in self:
            line.amount = line.currency_id.round(
                line.tax_amount * line.withholding_iva_id.iva_rate_id.name / 100
            )
