		...
		for partner in self:
			if partner.type in ('contact', 'invoice'):
				if partner.is_company:
					...
					if partner.person_type_id.code == 'PJDO' and partner.country_id.code == 'VE' and not (rif_regex.match(partner.vat) and self.simple_vat_check('ve', partner.vat)):
						raise ValidationError("El campo 'RIF' tiene un formato incorrecto.\nFormatos admitidos: J-12345678-9, V-12345678-9 o G-12345678-9.")
				else:
					...
					if partner.person_type_id.code == 'PNRE' and partner.country_id.code == 'VE' and not ci_regex.match(partner.identification):
						raise ValidationError("El campo 'Documento de identidad' tiene un formato incorrecto.\nFormatos admitidos: V12345678, E12345678, P1234567890.")
