...
class AccountMove(models.Model):
	...
	withholding_islr_id = fields.Many2one('account.withholding.islr', string='Retencion de ISLR', readonly=True, copy=False)
		...
		for move in to_post:
			if move._apply_withholding_islr():
				...
				if move.withholding_islr_id:
					...
					move.withholding_islr_id = self.env['account.withholding.islr'].create(values)
	...
	def _apply_withholding_islr(self):
		self.ensure_one()
		return self.invoice_line_ids.filtered(lambda l: l.product_id.apply_withholding_islr) and ((
			self.is_sale_document() and self.partner_id.is_islr_agent and not self.company_id.partner_id.is_islr_exempt
		) or (
			self.is_purchase_document() and self.company_id.partner_id.is_islr_agent and not self.partner_id.is_islr_exempt
		))

	def _get_withholding_islr_properties(self):
		self.ensure_one()

		field_balance = self.company_id.fiscal_currency_id.id == self.company_currency_id.id and 'balance' or 'balance_ref'
		concepts = defaultdict(float)
		for line in self.invoice_line_ids:
			if line.product_id.islr_concept_id:
				concepts[line.product_id.islr_concept_id] += abs(getattr(line, field_balance))

		person_type_id = self.partner_id.is_a_society and self.env.ref('l10n_ve_fiscal_identification.person_type_pnre') or self.partner_id.person_type_id
		ut = self.env['account.ut'].get_current_ut()
		line_ids = [Command.clear()]

		for concept, base in concepts.items():
			rate_id = concept.get_concept_rate(person_type_id, base_ut=(base / ut.amount))
			subtraction = rate_id['subtraction'] * ut.amount
			line_ids.append(Command.create({
				'islr_concept_id': concept.id,
				'rate_id': rate_id['id'],
				'base_amount': base,
				'amount': base * rate_id['base'] * rate_id['rate'] - subtraction,
