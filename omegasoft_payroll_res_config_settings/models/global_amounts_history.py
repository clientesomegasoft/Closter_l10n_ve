...
class MinimumWageHistory(models.Model):
    ...
    def write(self, vals):
        ...
        if line:
            ...
            self.search([('in_use', '=', True),('id', '!=', self.id)], limit=1).in_use = False
            if not line.in_use:
                line.in_use = True
    ...
    def set_minimum_wage(self):
        ...
        for company in self.env['res.company'].search([]):
            line = self.search([('date', '<=', date.today()),('company_id', '=', company.id)], order="date desc", limit=1)
            if line:
                line.company_id.write({
                    'minimum_wage': line.amount,
                    'minimum_wage_currency': line.currency_id.id
                })
                self.search([('in_use', '=', True),('id', '!=', line.id)], limit=1).in_use = False
                if not line.in_use:
                    line.in_use = True
    ...
    def _onchange_date(self):
        if self.date < date.today():
            raise ValidationError(_("It is not possible to create backdated records through the interface. Historical data must be entered by import upload."))
        
    @api.constrains('date')
    def _constrains_date(self):
        if self.search([('date', '=', self.date),('id', '!=', self.id)], order="date desc", limit=1):
