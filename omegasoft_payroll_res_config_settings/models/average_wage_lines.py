    ...
    def _check_number_of_best_weeks(self):
        for record in self:
            if record.number_of_best_weeks <= 0 and record.best_weeks_check:
                raise ValidationError('Cantidad de mejores semanas debe ser mayor a cero.')
            
    @api.constrains('number_of_weeks')
    def _check_number_of_weeks(self):
        for record in self:
            if record.number_of_weeks <= 0 and record.structure_type_default_schedule_pay == 'weekly':
                raise ValidationError('Cantidad de semanas a tomar debe ser mayor a cero.')