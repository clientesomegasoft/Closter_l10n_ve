...
class HrHCMCoverage(models.Model):
    ...
    total_quota_amount = fields.Monetary('Total quota amount', currency_field = 'currency_id')
    total_policy_coverage = fields.Monetary('Total policy coverage', currency_field='currency_id')
    ...
    def _compute_quotas(self):
        for record in self:
            record.company_quota_amount  = (record.total_quota_amount * (record.company_percentage_contribution / 100.0))
            record.employee_quota_amount = (record.total_quota_amount - record.company_quota_amount)
    ...
    def _get_default_currency_id(self):
        ...
        if not ves:
            ...
            _logger.warning('USING env.company.currency_id {ves} as default value for hr.hcm.coverage.scale')
        return ves
    ...
    def _constrains_company_percentage_contribution(self):
        for record in self:
            compare = lambda a, b: float_compare(a, b, precision_digits = record.currency_id.decimal_places)
            if compare(record.company_percentage_contribution, 100) > 0:
                raise ValidationError('Company Percentage is too high (max 100)')
            elif compare(record.company_percentage_contribution, 0) < 0:
                raise ValidationError('Company Percentage is too low (min 0)')
