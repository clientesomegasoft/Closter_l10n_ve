...
class EmployeeChart(http.Controller):
    @http.route(
        "/get/parent/colspan", type="json", auth="public", methods=["POST"], csrf=False
    )
    ...
    @http.route(
        "/get/parent/employee", type="json", auth="public", methods=["POST"], csrf=False
    )
    def get_employee_ids(self):
        employees = (
            request.env["hr.employee"].sudo().search([("parent_id", "=", False)])
        )
        names = []
        key = []
        if len(employees) == 1:
        ...
        if child_ids:
            ...
            for child in child_ids:
                ...
                view = (
                    ...
                    <div id='"""
                    + str(child.id)
                    + """' class="o_employee_border">
                    <img src='/web/image/hr.employee.public/"""
                    + str(child.id)
                    + """/image_1024/'/></div>
                    <div class='employee_name'><p>"""
                    + str(child.name)
                    + """</p>
                    <p>"""
                    + str(child.job_id.name)
                    + """</p></div></a></div>"""
                )
    ...
    @http.route(
        "/get/parent/child", type="http", auth="user", methods=["POST"], csrf=False
    )
    def get_parent_child(self, **post):
        if post:
            ...
            view = (
                """ <div id="parent" class='o_chart_head'><a>
                <div id='"""
                + str(val)
                + """' class="o_employee_border">
                <img class='o_emp_active' src='/web/image/hr.employee.public/"""
                + str(val)
                + """/image_1024/'/></div>
                <div class='employee_name o_width'><p>"""
                + str(emp.name)
                + """</p>
                <p>"""
                + str(emp.job_id.name)
    ...
    @http.route(
        "/get/child/data", type="json", auth="user", methods=["POST"], csrf=False
    )
