...
class ContractEmployeeSeniority(models.Model):
    ...
    days_of_seniority = fields.Float(
        string="Days of seniority",
        help="Indicates the days of employee seniority.",
        tracking=True,
    )

    months_of_seniority = fields.Float(
        string="Months of seniority",
        help="Indicates the months of employee seniority.",
        tracking=True,
    )

    years_of_seniority = fields.Float(
        string="Years of seniority",
        help="Indicates the years of employee seniority.",
        tracking=True,
    )
    ...
    def _get_current_contract(self, employee):
        ...
        date_current_contract = (
            max(actives_contracts.mapped("date_start")) if actives_contracts else False
        )
    ...
    def _employee_seniority(self):
        ...
        employee_obj = (
            ...
            .search([("active", "=", True)])
            .filtered(lambda self: self.contracts_count >= 1)
        )
        for employee in employee_obj:
            ...
            if current_contract and not current_contract.date_end:
                ...
                seniority = relativedelta(
                    current_contract.date_end, current_contract.date_start
                )
            if seniority:
