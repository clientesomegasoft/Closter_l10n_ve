...
class AriEmployeeSetting(models.Model):
    ...
    fiscal_year = fields.Integer(
        string="Año gravable", default=fields.Date.today().year, required=True
    )
    ...
    _sql_constraints = [
        (
            "unique_employee_per_year",
            "UNIQUE(fiscal_year, employee_id)",
            "Ya existe un registro del empleado para el año gravable seleccionado !!!",
        )
    ]
    ...
    def _onchange_employee(self):
        if self.employee_file:
            if self.employee_id:
                self.write(
                    {"employee_file_code_id": self.employee_id.employee_file_code_id.id}
                )
...
class AriEmployeeSettingLines(models.Model):
    ...
    _sql_constraints = [
        ...
        (
            ...
            "CHECK(amount > 0.0)",
            "El monto de cada linea debe ser mayor a cero.",
        ),
    ]
...
class HrEmployeeARI(models.Model):
    ...
    _sql_constraints = [
        (
            "unique_line_per_trimester",
            "UNIQUE(employee_id, fiscal_year, trimester)",
            "Ya existen lineas generadas en este trimestre para los empleados.",
        ),
    ]
