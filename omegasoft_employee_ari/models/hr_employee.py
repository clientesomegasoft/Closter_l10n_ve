...
class HrEmployeePrivate(models.Model):
	...
	employee_ari_ids = fields.One2many('hr.employee.ari', 'employee_id', string='ARI', readonly=True)
	employee_ari_setting_ids = fields.One2many('ari.employee.setting.lines', 'employee_id', string='Lineas')

	def _calculate_ari(self, force=False):
		def _get_trimester_field(month):
			return {1: 'trimester_1', 2: 'trimester_2', 3: 'trimester_3', 4: 'trimester_4'}[(month-1) // 3 + 1]
		...
		today = force and fields.Date.from_string(force) or fields.Date.context_today(self)
		if not any(today.day == day and today.month == month for day, month in PERIOD_DATES):
			return

		date_from, date_to = date_utils.get_quarter(today)
		employee_ids = self.search([('contract_id.state', '=', 'open'), ('company_id', '=', self.env.company.id)])
		...
		for employee_id in employee_ids:
			...
			family_burden = len(employee_id.family_information_ids.filtered(lambda l: l.is_family_burden))
			line = {
			...
			line.update(employee_id.employee_ari_setting_ids._get_ari_setting_by_trimester(trimester))

			wage_ut = wage / ut.amount
			to_hold_ut = _compute_factor(wage_ut - ((sum(line.get(key, 0.0) for key in  ('education', 'HCM', 'odontology', 'house')) / ut.amount) or 774.0)) - ((family_burden + 1) * 10) - (line.get('extra', 0.0) / ut.amount)
			percentage = to_hold_ut / wage_ut * 100

			last_trimester = employee_id.employee_ari_ids[-1:]
			if last_trimester and last_trimester.fiscal_year == today.year and last_trimester.percentage != percentage:
				percentage = ((to_hold_ut * ut.amount - withholding_amount.get(employee_id.id, 0.0)) / (wage - received_wage.get(employee_id.id, 0.0))) * 100
	...
	def _get_full_wage(self, date_from, date_to):
		...
		for employee_id in self:
			...
			if contract.wage_type == 'hourly':
				daily_wage = contract.custom_hourly_wage_currency._convert(contract.hourly_wage, company.currency_id, company, date_to) * 8
			else:
				daily_wage = contract.custom_wage_currency._convert(contract.wage, company.currency_id, company, date_to) / 30
			full_wage[employee_id.id] = (daily_wage + overtime.get(employee_id.id, 0.0)) * total_days
		return full_wage
	...
	def _get_employee_overtime(self, date_from, date_to):
		...
		if self.env.company.overtime_category_ids:
			...
			""",(tuple(self.ids), date_from, date_to, tuple(self.env.company.overtime_category_ids.ids), self.env.company.id))
		...
		if self.env.company.wage_received_rule_ids:
			self._cr.execute("""
				...
				WHERE hr_payslip.id = hr_payslip_line.slip_id
					...
					AND hr_payslip.company_id = %s
				GROUP BY hr_payslip.employee_id
			""",(tuple(self.ids), date_from, date_to, tuple(self.env.company.wage_received_rule_ids.mapped('code')), self.env.company.id))
		...
		if self.env.company.withholdig_rule_ids:
			self._cr.execute("""
				...
				WHERE hr_payslip.id = hr_payslip_line.slip_id
					...
					AND hr_payslip.company_id = %s
				GROUP BY hr_payslip.employee_id
			""",(tuple(self.ids), date_from, date_to, tuple(self.env.company.withholdig_rule_ids.mapped('code')), self.env.company.id))
...
class Contract(models.Model):
	...
	percentage_income_tax_islr = fields.Float(compute='_compute_percentage_income_tax_islr', store=True)

	@api.depends('employee_id.employee_ari_ids')
	def _compute_percentage_income_tax_islr(self):
		for rec in self:
			rec.percentage_income_tax_islr = rec.employee_id.employee_ari_ids[-1:].percentage