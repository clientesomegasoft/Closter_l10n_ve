...
class HrEmployeeAdvancesLoansDiscounts(models.Model):
    ...
    name = fields.Selection([('employee', 'Employee'), ('department', 'Department'), ('company', 'Company')], string="Advance type",
                            help="It refers to whether the Advance, Loan or discount is for an employee, specific department or the entire company.", tracking=True)

    date_issue = fields.Date(string='Date of issue', help="Date of issue of the advance, loan or discount", default=fields.Date.context_today, tracking=True)

    employee_ids = fields.Many2many( string='Employees', comodel_name='hr.employee', help="Employees", tracking=True, ondelete='restrict', domain= lambda self: self.env['hr.employee']._get_employee_l10n_ve_payroll_domain())
    ...
    employee_contract_ids = fields.Many2many(comodel_name='hr.contract',string='Employee contract',ondelete='restrict', tracking=True,)
    department_ids = fields.Many2many(comodel_name='hr.department', ondelete='restrict', string='Departments',tracking=True)

    employee_reference = fields.Char(string="Employee reference", help="reference of the loan or advance to be requested.", tracking=True)
    ...
    state = fields.Selection(selection=[
        ...
        ('paid', 'Paid'),
        ('rejected','Rejected'),
    ], string='Status', required=True, readonly=True, copy=False, tracking=True, store=True,
        default='draft')
    ...
    advances_loans_discounts_line_ids = fields.One2many(comodel_name='hr_employee_advances_loans_discounts_lines', inverse_name='advances_loans_discounts_id', string='Employee Advances, loans and discounts lines')

    def get_employee_advance_domain(self):
        employee_obj = self.employee_ids if self.employee_ids else self.env['hr.employee']
        return self.env['hr.employee'].search(employee_obj._get_employee_l10n_ve_payroll_domain())
    ...
    def write(self, vals):
        ...
        for record in self.employee_ids:
            ...
            if employee_contract.state == 'open' and self.state in ['open']:
                ...
                record._compute_advance_benefit_interest(employee_contract, self.advances_loans_discounts_line_ids)
                record._compute_advance_days_per_year(employee_contract, self.advances_loans_discounts_line_ids)
        return res
    ...
    def _dispatch_hr_data_from_employee_ids(self):
        ...
        if self.employee_ids:
            employee_contract_ids.append( fields.Command.set(self.employee_ids.contract_ids.ids) )
            department_ids.append( fields.Command.set(self.employee_ids.department_id.ids) )
    ...
    def _dispatch_hr_data_from_department_ids(self):
        ...
        if self.department_ids:
            maybe_employees = self.get_employee_advance_domain().filtered(lambda x: x.id in self.department_ids.member_ids.ids)
            if not maybe_employees:
                raise ValidationError("El departamento seleccionado no tiene empleados con contratos En Proceso")

        employee_ids.append(fields.Command.set(maybe_employees.ids))
        employee_contract_ids.append(fields.Command.set(maybe_employees.contract_ids.ids))
    ...
    def action_cancel(self):
        ...
        for record in self.advances_loans_discounts_line_ids:
            ...
            record.product_employee_ids._unlink_advance_loans_discounts(record.product_employee_ids.contract_id, record)
    ...
    def action_draft(self):
        ...
        for record in self.advances_loans_discounts_line_ids:
            ...
            record.employee_ids._unlink_advance_loans_discounts(record.product_employee_ids.contract_id, record)
    ...
    def _constrain_advances_loans_discounts_line_ids(self):
        ...
        if self.advances_loans_discounts_line_ids:
            for record in self.advances_loans_discounts_line_ids:
                if record.amount <= 0:
                    raise ValidationError('El monto en las lineas debe ser mayor a cero.')
                if record.type_advance_loan in ['loan','discount','per_diem'] and record.fees <= 0:
                    raise ValidationError('Las Cuotas en las lineas no pueden ser menor o igual a cero')
    ...
    def _compute_file_code_ids(self):
        if self.employee_file:
            for record in self:
                record.employee_file_code_ids = [ fields.Command.clear(), fields.Command.set(record.employee_ids.employee_file_code_id.ids) ]

    @api.onchange('employee_file_code_ids')
    def _inverse_file_code_ids(self):
        if self.employee_file:
            for record in self:
                record.employee_ids = [ fields.Command.clear(), fields.Command.set(record.employee_file_code_ids.employee_id.ids) ]

    @api.constrains('employee_file_code_ids')
    def _constrains_employee_file_code_ids(self):
        if self.employee_file:
            if not self._context.get('bypass', False):
                if self.employee_file_code_ids and self.employee_file_code_ids.employee_id.ids != self.employee_ids.ids:
                    self.employee_ids = [(6, 0, self.employee_file_code_ids.employee_id.ids)]
    ...
    def _constrains_employee(self):
        if self.employee_file:
            if self.employee_ids and self.employee_ids.ids != self.employee_file_code_ids.employee_id.ids:
                self.employee_file_code_ids = [(6, 0, self.employee_ids.employee_file_code_id.ids)]
