...
class CommissionConfLine(models.Model):
	...
	department_id = fields.Many2one('hr.department', string="Department", required=True, domain=[('generate_commissions', '=', True)])
	bill_calculation = fields.Selection([
		('posted_bill', 'Invoice generated'),
		('paid_bill', 'Invoice paid')
	], string="Bill Calculation", required=True)
	...
	def _onchange_direct_percentage(self):
		if self.direct_percentage < 0 or self.direct_percentage > 100:
			raise ValidationError(_("You must enter percentages greater than 0 or less than 100."))
	...
	def _onchange_department_id(self):
		department_ids = self.contract_id.commission_conf_line_ids.mapped('department_id')
		return {'domain': {'department_id': [('generate_commissions', '=', True), ('id', 'not in', department_ids.ids)]}} 
