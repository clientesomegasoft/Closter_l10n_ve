...
class ProductProduct(models.Model):
    ...
    sale_avg_price_ref = fields.Float(
        compute="_compute_product_margin_fields_values",
        string="Precio Unidad Promedio de Venta op.",
    )
    turnover_ref = fields.Float(
        compute="_compute_product_margin_fields_values", string="Volumen de negocio op."
    )
    sales_gap_ref = fields.Float(
        compute="_compute_product_margin_fields_values", string="Diferencia ventas op."
    )
    total_cost_ref = fields.Float(
        compute="_compute_product_margin_fields_values", string="Coste total op."
    )
    total_margin_ref = fields.Float(
        compute="_compute_product_margin_fields_values", string="Margen total op."
    )
    ...
    def _compute_product_margin_fields_values(self, field_names=None):
        ...
        res = {
            product_id: {
                ...
                "sale_avg_price_ref": 0.0,
                "turnover_ref": 0.0,
                "sales_gap_ref": 0.0,
                "total_cost_ref": 0.0,
                "total_margin_ref": 0.0,
            }
        ...
        for row in self.env.cr.dictfetchall():
            ...
            res[product_id]["sales_gap_ref"] = (row["sale_expected_ref"] or 0.0) - res[
                product_id
            ]["turnover_ref"]
        ...
        for row in self.env.cr.dictfetchall():
            ...
            res[product_id]["total_margin_ref"] = (
                res[product_id].get("turnover_ref", 0.0)
                - res[product_id]["total_cost_ref"]
            )
