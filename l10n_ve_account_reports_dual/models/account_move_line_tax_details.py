...
class AccountMoveLine(models.Model):
    ...
    def _get_query_tax_details(self, tables, where_clause, where_params, fallback=True):
        ...
        if group_taxes_query_list:
            group_taxes_query = f"""
            UNNEST(
                CASE {' '.join(group_taxes_query_list)}
                 ELSE ARRAY[tax.id]
                END
            )"""
        else:
        ...
        if fallback:
            fallback_query = f"""
                ...
                SELECT
                    ...
                    {"base_line.balance AS base_amount," if not self._context.get('ref_report') else "base_line.balance_ref AS base_amount,"}
                ...
                JOIN account_move_line_account_tax_rel tax_rel ON
                    tax_rel.account_tax_id = COALESCE(
                        account_move_line.group_tax_id, account_move_line.tax_line_id)
        ...
        return (
            f"""
            /*
            As example to explain the different parts of the query, we'll consider
            a move with the following lines:
            ...
            */
            ...
            WITH affecting_base_tax_ids AS (
                ...
                /*
                This CTE builds a reference table based on the tax_ids field, with
                the following changes:
                  - flatten the group of taxes
                  - exclude the taxes having 'is_base_affected' set to False.
                Those allow to match only base_line_1 when finding the base lines
                of tax_line_1, as we need to find base lines having a 'affecting_base_tax_ids'
                ending with [10_affect_base, 20], not only containing '10_affect_base'.
                Otherwise, base_line_2/3 would also be matched. In our example, as all the
                taxes are set to be affected by previous ones affecting the base, the
                ...
                */
                ...
                SELECT
                    sub.line_id AS id,
                    ARRAY_AGG(
                        sub.tax_id ORDER BY sub.sequence, sub.tax_id
                    ) AS tax_ids
                FROM (
                    SELECT
            ...
            base_tax_line_mapping AS (
                ...
                /*
                Create the mapping of each tax lines with their
                ...
                */
                ...
                SELECT
                    ...
                    base_line.id AS base_line_id,
                    {
                        ...
                        if not self._context.get('ref_report')
                        else "base_line.balance_ref AS base_amount,"
                    }
                    {
                        "base_line.amount_currency AS base_amount_currency"
                        if not self._context.get('ref_report')
                        else "base_line.balance_ref AS base_amount_currency"
                    }
                ...
                JOIN account_move_line base_line ON
                    ...
                    AND (
                        ...
                        if not self._context.get('ref_report') else
                        "SIGN(account_move_line.balance_ref) = SIGN(base_line.balance_ref * tax.amount * tax_rep.factor_percent)"}
                    )
                    AND COALESCE(base_line.partner_id, 0) = COALESCE(
                        account_move_line.partner_id, 0)
                    AND base_line.currency_id = account_move_line.currency_id
                    AND (
                        COALESCE(
                            tax_rep.account_id,
                    ...
                    AND (
                        NOT tax.analytic
                        OR (base_line.analytic_distribution IS NULL
                        AND account_move_line.analytic_distribution IS NULL
                    )
                OR base_line.analytic_distribution = account_move_line.analytic_distribution
                    )
                LEFT JOIN affecting_base_tax_ids tax_line_tax_ids
                ON tax_line_tax_ids.id = account_move_line.id
                JOIN affecting_base_tax_ids base_line_tax_ids
                ON base_line_tax_ids.id = base_line.id
                WHERE account_move_line.tax_repartition_line_id IS NOT NULL
                    AND {where_clause}
                    AND (
                        -- keeping only the rows from affecting_base_tax_lines
                        -- that end with the same taxes
                        -- applied (see comment in affecting_base_tax_ids)
                        NOT tax.include_base_amount
                        OR base_line_tax_ids.tax_ids[
                            ARRAY_LENGTH(base_line_tax_ids.tax_ids, 1) - COALESCE(
                            ARRAY_LENGTH(tax_line_tax_ids.tax_ids, 1),
                            0
                        ):ARRAY_LENGTH(base_line_tax_ids.tax_ids, 1)]
                            = ARRAY[account_move_line.tax_line_id] || COALESCE(
                                tax_line_tax_ids.tax_ids, ARRAY[]::INTEGER[])
                    )
            ),
            ...
            tax_amount_affecting_base_to_dispatch AS (
                ...
                /*
                Computes the total amount to dispatch in case of tax lines affecting
                the base of subsequent taxes. Such tax lines are an additional base
                amount for others lines, that will be truly dispatch in next CTE.

                In the example:
                    - tax_line_1 is an additional base of 100.0 from
                      base_line_1 for tax_line_2.
                    - tax_line_3 is an additional base of 2/5 * 500.0 = 200.0
                      from base_line_2 for tax_line_4.
                    - tax_line_3 is an additional base of 3/5 * 500.0 = 300.0
                      from base_line_3 for tax_line_4.
                ...
                */
                ...
                SELECT
                    ...
                    SUM(
                        ...
                        "THEN CASE WHEN base_line.balance < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.balance" if not self._context.get('ref_report')
                        else "THEN CASE WHEN base_line.balance_ref < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.balance_ref"}
                        END
                    ) OVER (
                        PARTITION BY tax_line.id, account_move_line.id
                        ORDER BY tax_line.tax_line_id, base_line.id) AS cumulated_base_amount,
                    SUM(
                        CASE
                         WHEN tax.amount_type = 'fixed'{
                        "THEN CASE WHEN base_line.balance < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.balance" if not self._context.get('ref_report')
                        else "THEN CASE WHEN base_line.balance_ref < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.balance_ref"}
                        END
                    ) OVER (
                    ...
                    ) AS total_base_amount,
                    {
                        ...
                        if not self._context.get('ref_report')
                        else "account_move_line.balance_ref AS total_tax_amount,"
                    }

                    {
                        "base_line.amount_currency AS base_amount_currency,"
                        if not self._context.get('ref_report')
                        else "account_move_line.balance_ref AS base_amount_currency,"
                    }
                    SUM(
                        CASE WHEN tax.amount_type = 'fixed'
                        {"THEN CASE WHEN base_line.amount_currency < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.amount_currency" if not self._context.get('ref_report')
                        else "THEN CASE WHEN base_line.balance_ref < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.balance_ref"}
                        END
                    ) OVER (
                        PARTITION BY tax_line.id, account_move_line.id
                        ORDER BY tax_line.tax_line_id, base_line.id
                    ) AS cumulated_base_amount_currency,
                    SUM(
                        CASE
                         WHEN tax.amount_type = 'fixed'
                         {"THEN CASE WHEN base_line.amount_currency < 0 THEN -1 ELSE 1 END * ABS(COALESCE(base_line.quantity, 1.0)) ELSE base_line.amount_currency" if not self._context.get('ref_report')
                        ...
                        PARTITION BY tax_line.id, account_move_line.id
                    ) AS total_base_amount_currency,
                    {
                        "account_move_line.amount_currency AS total_tax_amount_currency"
                        if not self._context.get('ref_report')
                        else "account_move_line.balance_ref AS total_tax_amount_currency"
                    }
            ...
            base_tax_matching_base_amounts AS (
                ...
                /*
                Build here the full mapping tax lines <=> base lines containing the
                final base amounts. This is done in a 3-parts union.
                ...
                */
                ...
                /*
                PART 2: Dispatch the tax amount of tax lines affecting the base of
                subsequent ones, using tax_amount_affecting_base_to_dispatch.
                ...
                */
                ...
                SELECT
                    ...
                    ROUND(
                        COALESCE(
                            SIGN(sub.cumulated_base_amount) * sub.total_tax_amount *
                            ABS(sub.cumulated_base_amount) / NULLIF(sub.total_base_amount, 0.0),
                            0.0
                        ),
                        sub.comp_curr_prec
                    )
                    ...
                    OVER (
                        PARTITION BY sub.tax_line_id, sub.src_line_id
                    ...
                    ROUND(
                        COALESCE(
                            SIGN(
                                sub.cumulated_base_amount_currency
                            ) * sub.total_tax_amount_currency *
                            ABS(sub.cumulated_base_amount_currency) / NULLIF(
                                sub.total_base_amount_currency, 0.0),
                            0.0
                        ),
                        sub.curr_prec
                    )
                ...
                /*
                PART 3: In case of the matching failed because the configuration changed or
                some journal entries have been imported, construct a simple mapping as a
                fallback. This mapping is super naive and only build based on the 'tax_ids'
                and 'tax_line_id' fields, nothing else. Hence, the mapping will not be exact
                but will give an acceptable approximation.
                ...
                SELECT
                    ...
                    SUM(
                        CASE
                         ...
                         THEN
                          CASE
                           WHEN base_line.balance < 0
                           THEN -1
                           ELSE 1
                          END * ABS(COALESCE(base_line.quantity, 1.0))
                         ELSE sub.base_amount
                        END
                    ) OVER (
                        PARTITION BY tax_line.id
                        ORDER BY tax_line.tax_line_id, sub.base_line_id, sub.src_line_id
                    ) AS cumulated_base_amount,
                    SUM(
                        CASE
                         WHEN tax.amount_type = 'fixed'
                         THEN
                        ...
                        PARTITION BY tax_line.id) AS total_base_amount,
                    {
                        "tax_line.balance AS total_tax_amount,"
                        if not self._context.get('ref_report') else
                        "tax_line.balance_ref AS total_tax_amount,"
                    }
                    ...
                    SUM(
                        CASE
                         ...
                         THEN
                          CASE
                           WHEN base_line.amount_currency < 0
                           THEN -1
                           ELSE 1
                          END * ABS(COALESCE(base_line.quantity, 1.0)
                          )
                    ...
                    ) AS total_base_amount_currency,
                    {
                        ...
                        if not self._context.get('ref_report') else
                        "tax_line.balance_ref AS total_tax_amount_currency"
                    }
           ...
           /* Final select that makes sure to deal with rounding errors, using LAG
           to dispatch the last cents. */
