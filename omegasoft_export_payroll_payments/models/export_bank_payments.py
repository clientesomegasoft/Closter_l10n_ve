...
class ExportBankPayments(models.Model):
    ...
    bank_account_id = fields.Many2one('res.partner.bank', 'Número de Cuenta Bancaria', states=_READONLY_STATES,
                                    help="Cuenta bancaria de la compañía de donde será debitado el dinero.")
    by_employee = fields.Boolean(string="Por empleado", default=False,
                                help="Indica si el txt se genera para determinados empleados.", states=_READONLY_STATES)
    employee_ids = fields.Many2many('hr.employee', string='Empleados', states=_READONLY_STATES,
                                    help="Empleados para los que se genera el archivo txt.")
    employee_domain = fields.Many2many('hr.employee', compute="_compute_employee_ids",
                                    string='Dominio para Empleados', help="Dominio para filtrar empleados.")
    description = fields.Char(
        'Descripción', help="Texto descriptivo asociado al archivo txt generado.", states=_READONLY_STATES)
    
    structure_id = fields.Many2one('hr.payroll.structure', string='Tipo de nómina', states=_READONLY_STATES,
                                help="Tipo de nómina a tomar en cuenta para la generación del archivo txt.")
    operation_type = fields.Selection([('same', 'Mismo banco'), ('other', 'Otros bancos')], string="Tipo de operación",
                                    states=_READONLY_STATES, default='same', help="Indica si el pago de nominas es entre cuentas del mismo banco o no.")
    ...
    txt_type = fields.Selection([
        ('ticket', 'Todoticket'),
        ('guard', 'Guarderia')
    ], string='Tipo TXT', states=_READONLY_STATES)

    valid_date = fields.Date(
        'Fecha efectiva de pago', states=_READONLY_STATES, help="Fecha en que se ejecutará el pago.")
    ...
    def _get_employee_domain(self, bank_account_number, operation_type_same=False):
        ...
        if operation_type_same:
            employees_bank_info = self.env['hr_employee_bank_information'].search([('is_payroll_account', '=', True)]).filtered(lambda x: x.name == self.bank_id)
        else:
            employees_bank_info = self.env['hr_employee_bank_information'].search([('is_payroll_account', '=', True)]).filtered(lambda x: x.name != self.bank_id)
        
        employees_domain = self.env['hr.employee'].search([('id', 'in', employees_bank_info.employee_id.ids)])
        if self.structure_id:
            payslip_employees = self.env['hr.payslip'].search([
        ...
        if payslip_employees:
            employees_domain = employees_domain.filtered(lambda e: e.id in payslip_employees.ids)
        else:
            employees_domain = False
    ...
    def _onchange_bank_account_id(self):
        ...
        return {'domain': {'bank_account_id': [('id', 'in', banks_ids), ('bank_id', '=', self.bank_id.id)]}}
    ...
    def _compute_employee_ids(self):
        for export_bank_payment in self:
            ...
            if export_bank_payment.bank_account_id and export_bank_payment.by_employee:
                bank_account_number = export_bank_payment.bank_account_id.sanitized_acc_number[:4]
                
                #Obtener los ids de los empleados
                if export_bank_payment.operation_type == 'same':
                    employees_domain = self._get_employee_domain(bank_account_number, True)
                    export_bank_payment.employee_domain = employees_domain
                else:
                    employees_domain = self._get_employee_domain(bank_account_number)
                    export_bank_payment.employee_domain = employees_domain

            elif export_bank_payment.type_trans == 'fiscal' and export_bank_payment.by_employee:
    ...
    def unlink(self):
        sequences = self.env.ref('omegasoft_export_payroll_payments.sequence_txt_bank_rolda')
        current_sequence = self.env['export.bank.payments'].search([], order='name desc', limit=1)
        
        if all(int(item) < int(current_sequence.name) for item in self.mapped('name')):
            raise ValidationError("No se puede eliminar. Hay secuencias superiores creadas")
        else:
            if len(self) == 1:
    ...
    def action_done(self):
        ...
        if self.env.company.partner_id.partner_type != 'other' and not self.env.company.partner_id.vat:
            raise ValidationError(
        ...
        if self.type_trans != 'fiscal':
            ...
            if self.operation_type == 'same':
                ...
                if not employee_domain:
                    raise ValidationError('No se pudo generar el archivo. Intente de nuevo con otro periodo.')
                payslips = payslips.filtered(
                    lambda x: self.bank_account_id.acc_number and x.employee_id.id in employee_domain.ids)
            else:
    ...
    def employee_payroll_validations(self, payslip):
        ...
        if not payslip.net_wage or payslip.net_wage <= 0.0:
            raise ValidationError(("Establezca un monto valido a pagar al empleado: %s.") % (
                payslip.employee_id.name))

        if len(f'{payslip.net_wage:.2f}') > 13:
            raise ValidationError(("El monto a pagar para el empleado: %s, excede la cantidad maxima de digitos.") % (
                payslip.employee_id.name))
        ...
        if self.operation_type == 'other':
            ...
            if not payslip.employee_id.country_id:
                raise ValidationError(("Por favor establezca la nacionalidad del empleado: %s.") % (
                    payslip.employee_id.name))
    ...
    def _txt_prepare_data_bnc(self, payslip):
        ...
        employees_bank_info = self.env['hr_employee_bank_information'].search([('employee_id', '=', payslip.employee_id.id), ('is_payroll_account', '=', True)])
        ...
        if not employees_bank_info.letter:
            raise ValidationError("No se encuentra configurada la Letra Calificadora en los datos bancarios del empleado {}".format(employee.name))
    ...
    def _txt_prepare_data_banesco(self, payslip):
        ...
        aux_txt_data += '{:0>15}'.format(amount)
        aux_txt_data += "VES"
        aux_txt_data += '{0:<30}'.format(bank_acc)
        aux_txt_data += '{0:<11}'.format(bank_acc[:4])
        aux_txt_data += "   "
        aux_txt_data += '{0:<17}'.format(nationality + id_digits)
        aux_txt_data += '{0:<70}'.format(employee_name)
        aux_txt_data += ' ' * 70
        aux_txt_data += ' ' * 25
        aux_txt_data += ' ' * 17
        aux_txt_data += ' ' * 35
    ...
    def generate_banesco(self):
        ...
        if self.txt_type == 'ticket':
            ...
            # 1. Preparacion de campos
                ...
                txt_data += " " * 32
                txt_data += "9  "
                txt_data += '{0:<35}'.format(self.name)
                ...
                #débito
                txt_data += "02"
                txt_data += '{:0>8}'.format(self.name) + ' ' * 22
                txt_data += nationality.upper() + '{0:<16}'.format(vat_digits)
                txt_data += '{0:<35}'.format(self.env.company.partner_id.name)
                txt_data += '{:0>15}'.format(total_payroll)
                txt_data += "VES "
                txt_data += '{0:<34}'.format(bank_acc)
                txt_data += "BANESCO    "
                ...
                #Totales
                txt_data += "06"
                txt_data += '{:0>15}'.format(1)
                txt_data += '{:0>15}'.format(len(payslips))
                txt_data += '{:0>15}'.format(total_payroll)
    ...
    def _txt_prepare_data_vzla(self, payslip):
        ...
        aux_txt_data += nationality.upper() + '{:0<9}'.format(id_digits)
        aux_txt_data += '{0:<30}'.format(employee_name)
        aux_txt_data += "00" if employees_bank_info.account_type == 'current_account' else "01"
        aux_txt_data += bank_acc
        aux_txt_data += '{:0>18}'.format(amount)
        aux_txt_data += "10" if self.operation_type == "same" else 00 #Tipo de Pago
        aux_txt_data += '{0:<12}'.format(employees_bank_info.name.bic)
    ...
    def generate_vzla(self):
        ...
        if payslips:
            ...
            #Header
            txt_data += "HEADER"
            txt_data += '{:0>8}'.format(self.name)
            txt_data += '{0:<8}'.format('') #CAMBIAR POR NúMERO DE NEGOCIACIÓn
            txt_data += nationality.upper() + '{:0<9}'.format(vat_digits)
            ...
            txt_data += bank_acc
            txt_data += '{:0>18}'.format(total_payroll)
            txt_data += 'VEF'
            txt_data += '40' + '\n'
            ...
            #Totales
            txt_data += "TOTAL"
            txt_data += '{:0>5}'.format(1)
            txt_data += '{:0>5}'.format(len(payslips))
            txt_data += '{:0>18}'.format(total_payroll)
    ...
    def _txt_prepare_data_mercantil(self, payslip):
        ...
        aux_txt_data += '{:0>15}'.format(id_digits)
        aux_txt_data += '1' if self.operation_type == 'same' else '3'
        aux_txt_data += '{:0>12}'.format("")
        aux_txt_data += '{0:<30}'.format('')
        aux_txt_data += bank_acc
        aux_txt_data += '{:0>17}'.format(amount)
        aux_txt_data += '{0:<16}'.format(nationality + id_digits)
    ...
    def generate_mercantil(self):
        ...
        if payslips:
            ...
            txt_data += '{:0>15}'.format(self.name)
            txt_data += "NOMIN"
            txt_data += "0000000222"
            txt_data += nationality
            txt_data += '{:0>15}'.format(vat_digits)
            txt_data += '{:0>8}'.format(len(payslips))
            txt_data += '{:0>17}'.format(total_payroll)
            txt_data += self.valid_date.strftime("%Y%m%d")
            txt_data += bank_acc
            txt_data += '{:0>7}'.format("")
            txt_data += '{:0>8}'.format("")
