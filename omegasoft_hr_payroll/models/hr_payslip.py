...
class HrPayslip(models.Model):
    ...
    schedule_pay_contract = fields.Selection(
        related="contract_id.structure_type_id.default_schedule_pay"
    )
    ...
    average_wage = fields.Monetary(
        related="employee_id.average_wage", string="Average wage"
    )
    ...
    def _onchange_number_of_week(self):
        if (
            self.name
            and self.struct_id
            and self.contract_id.structure_type_id.default_schedule_pay == "weekly"
        ):
            self.week_number = (
                self.date_to.isocalendar()[1]
                if self.date_to.isocalendar()[1] == self.date_from.isocalendar()[1]
    ...
    def _onchange_date_week(self):
        if (
            ...
            self.date_from = datetime(1997, 1, 1) + relativedelta(
                weekday=MO(-1), weeks=int(self.week_number), year=self.date_from.year
            )
    ...
    def _onchange_fortnight(self):
        ...
        for record in self:
            if record.struct_id and record.struct_id.schedule_pay != "bi-weekly":
                ...
                record.date_to = fields.Date.to_string(
    ...
    def action_send_payslip_by_email(self):
        ...
        template = self.env.ref(
            "omegasoft_hr_payroll.mail_template_new_payslip_custom_omegasoft",
    ...
    def _compute_basic_net(self):
        for payslip in self:
            ...
            # rules BAS7
            codes_basic = (
                self.env.company.salary_rules_for_basic.filtered(
                    lambda struct: struct.struct_id.id == payslip.struct_id.id
                ).mapped("code")
                if self.env.company.salary_rules_for_basic
                else []
            )

            # rules NET
            codes_net = (
                self.env.company.salary_rules_for_net.filtered(
                    lambda struct: struct.struct_id.id == payslip.struct_id.id
                ).mapped("code")
                if self.env.company.salary_rules_for_net
                else []
            )
            codes = []
    ...
    def _compute_earnings_generated(self, is_payslip_cancel=False):
        structures_for_profits = (
            self.env.company.payroll_structure_for_profits.ids
            if self.env.company.payroll_structure_for_profits
            else []
        )
        categories_for_profits = (
            self.env.company.salary_rules_categories_for_profits.mapped("code")
            if self.env.company.salary_rules_categories_for_profits
            else []
        )
        for record in self:
            if record.struct_id.id in structures_for_profits:
                profits = sum(
                    item.total
                    for item in record.line_ids.filtered(
                        lambda x: x.category_id.code in categories_for_profits
                    )
                )
                total_profits = profits * (
                    (record.env.company.days_of_profit or 0) / 360
                )
                if not is_payslip_cancel:
                    record.contract_id.earnings_generated += total_profits
                else:
                    record.contract_id.earnings_generated -= (
                        total_profits
                        if record.contract_id.earnings_generated > 0
                        else 0
                    )
                record.contract_id.earnings_generated_total_available = (
                    record.contract_id.earnings_generated
                    - record.contract_id.advances_granted
                )
            else:
                pass

    def _compute_profits_resets(self, is_payslip_cancel=False):
        structures_for_profits_resets = (
            self.env.company.structures_for_utility_resets.ids
            if self.env.company.structures_for_utility_resets
            else []
        )
        for record in self:
            if (
                not is_payslip_cancel
                and record.struct_id.id in structures_for_profits_resets
            ):
                record.contract_id.earnings_generated_previous_amount = (
                    record.contract_id.earnings_generated
                )
                record.contract_id.advances_granted_previous_amount = (
                    record.contract_id.advances_granted
                )
                ...
                payslip_obj = self.env["hr.payslip"].search(
                    [
                        ...
                        ("state", "in", ["done", "paid"]),
                        ("struct_id", "in", structures_for_profits_resets),
                    ],
                ...
                for payslib in payslip_obj:
                    if (
                        not previous_resets
                        and payslib.struct_id.id in structures_for_profits_resets
                        and record.id != payslib.id
                    ):
    ...
    def _compute_vacations_resets(self, is_payslip_cancel=False):
        structures_for_vacations_resets = (
            self.env.company.structures_for_utility_resets.ids
            if self.env.company.structures_for_utility_resets
            else []
        )
        for record in self:
            if (
                not is_payslip_cancel
                and record.struct_id.id in structures_for_vacations_resets
            ):
                record.contract_id.vacations_advances_granted_previous_amount = (
                    record.contract_id.vacations_advances_granted
                )
    ...
    def _compute_social_benefits_resets(self, is_payslip_cancel=False):
        structure_for_resets_labor_liabilities = (
            self.env.company.structure_for_resets_labor_liabilities.ids
            if self.env.company.structure_for_resets_labor_liabilities
            else []
        )
        for record in self:
            if (
                not is_payslip_cancel
                and record.struct_id.id in structure_for_resets_labor_liabilities
            ):
                record.contract_id.social_benefits_generated_previous_amount = (
                    record.contract_id.social_benefits_generated
                )
                record.contract_id.accrued_social_benefits_previous_amount = (
                    record.contract_id.accrued_social_benefits
                )
                record.contract_id.advances_of_social_benefits_previous_amount = (
                    record.contract_id.advances_of_social_benefits
                )

                record.contract_id.benefit_interest_previous_amount = (
                    record.contract_id.benefit_interest
                )
                record.contract_id.days_per_year_accumulated_previous_amount = (
                    record.contract_id.days_per_year_accumulated
                )
                ...
                record.contract_id.total_available_social_benefits_generated = (
                    record.contract_id.social_benefits_generated
                    + record.contract_id.accrued_social_benefits
                ) - record.contract_id.advances_of_social_benefits
    ...
    def _check_payroll(self):
        if (
            self.struct_id
            and self.payroll_structure_for_profits
            and self.struct_id.id in self.payroll_structure_for_profits.ids
        ):
            if (
                self.contract_id.recent_resets
                and self.contract_id.recent_resets >= self.date_from
            ):
    ...
    def _action_create_account_move(self):
        ...
        for slip in payslips_to_post:
            slip_mapped_data[slip.struct_id.journal_id.id][
                fields.Date().end_of(slip.date_to, "month")
            ] |= slip
        for journal_id in slip_mapped_data:  # For each journal_id.
            for slip_date in slip_mapped_data[journal_id]:  # For each month.
                ...
                for slip in slip_mapped_data[journal_id][slip_date]:
                    ...
                    move_dict["currency_rate_ref"] = (
                        slip.rate_id.id
                        if slip.rate_id
                        else self.env["res.currency.rate"]
                        .sudo()
                        .search([("is_payroll_rate", "=", True)], limit=1)
                        .id
                    )
    ...
    def _prepare_line_values(self, line, account_id, date, debit, credit):
        ...
        if self.currency_id != self.company_id.currency_id:
            ...
            debit_ref = self.currency_id._convert(
                debit, self.company_id.currency_id, self.company_id, date_ref
            )
