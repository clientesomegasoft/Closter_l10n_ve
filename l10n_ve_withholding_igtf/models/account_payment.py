...
class AccountPayment(models.Model):
    ...
    def action_post(self):
        ...
        if (
            not self.apply_igtf
            or not self.calculate_igtf
            or self.currency_id.is_zero(self.igtf_amount)
        ):
        ...
        available_payment_method_line_ids = journal._get_available_payment_method_lines(
            self.payment_type
        )
        ...
        if self.payment_type == "inbound":
            ...
            outstanding_account_id = (
                ...
                and available_payment_method_line_ids[0].payment_account_id
        ...
        if self.currency_id == self.company_currency_id:
            ...
            liquidity_balance = self.company_currency_id.round(
                liquidity_amount_currency / self.currency_rate_ref.rate
            )
        else:
        ...
        if not outstanding_account_id:
            raise ValidationError(
                "No puede crear un nuevo pago sin una cuenta de pagos/recibos pendientes establecida en ya sea la empresa o el diario IGTF"
            )
    ...
    def _seek_for_igtf_lines(self):
        ...
        for line in self.igtf_move_id.line_ids:
            ...
            elif line.account_id in (
                self.company_id.igtf_inbound_account_id,
                self.company_id.igtf_outbound_account_id,
            ):
